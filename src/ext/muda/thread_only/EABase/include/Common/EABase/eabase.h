...
// The case is similar with wchar_t under C++.
    ...
    || defined(EA_WCHAR_T_NON_NATIVE) || defined(EA_PLATFORM_SONY)
...
#ifndef INT8_C
#define INT8_C(x)                                                              \
    int8_t(x)  // For the majority of compilers and platforms, long is 32 bits and long long is 64 bits.
#endif
#ifndef UINT8_C
...
#define EA_CHAR16_NATIVE 0
    ...
    && defined(__CHAR16_TYPE__) && defined(EA_COMPILER_CPP11_ENABLED)  // EDG 4.4+.
#define EA_CHAR16_NATIVE 1
#elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004)               \
    && !defined(EA_COMPILER_EDG)                                               \
    && (defined(EA_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__))  // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
#define EA_CHAR16_NATIVE 1
#else
#define EA_CHAR16_NATIVE 0
#endif
...
#endif
#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404)                     \
    && defined(__CHAR32_TYPE__) && defined(EA_COMPILER_CPP11_ENABLED)  // EDG 4.4+.
#define EA_CHAR32_NATIVE 1
#elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004)               \
    && !defined(EA_COMPILER_EDG)                                               \
    && (defined(EA_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__))  // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
...
MUDA_THREAD_ONLY char (&EAArraySizeHelper(T (&x)[N]))[N];
...
#define EA_IS_ENABLED(x)                                                       \
    (333 == 333 * 111 / ((x 0) * (((x 0) == 333 ? 1 : 0) + ((x 0) == 111 ? 1 : 0))))
