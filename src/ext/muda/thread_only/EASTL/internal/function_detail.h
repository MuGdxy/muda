...
namespace internal
{
    ...
    struct functor_storage
    {
        ...
        MUDA_THREAD_ONLY Ret& GetStorageTypeRef() const
        {
    ...
    class function_base_detail
    {
      public:
        ...
        {
          public:
            MUDA_THREAD_ONLY static Functor* GetFunctorPtr(const FunctorStorageType& storage) EA_NOEXCEPT
            {
                return &(storage.template GetStorageTypeRef<Functor>());
            }

            template <typename T>
            MUDA_THREAD_ONLY static void CreateFunctor(FunctorStorageType& storage, T&& functor)
            {
                ::new(GetFunctorPtr(storage)) Functor(eastl::forward<T>(functor));
            }

            MUDA_THREAD_ONLY static void DestructFunctor(FunctorStorageType& storage)
            {
                GetFunctorPtr(storage)->~Functor();
            }

            MUDA_THREAD_ONLY static void CopyFunctor(FunctorStorageType& to,
                                                     const FunctorStorageType& from)
            {
                ::new(GetFunctorPtr(to)) Functor(*GetFunctorPtr(from));
            }

            MUDA_THREAD_ONLY static void MoveFunctor(FunctorStorageType& to,
                                                     FunctorStorageType& from) EA_NOEXCEPT
            {
                ::new(GetFunctorPtr(to)) Functor(eastl::move(*GetFunctorPtr(from)));
            }

            MUDA_THREAD_ONLY static void* Manager(void* to,
                                                  void* from,
        ...
        class function_manager_base<Functor, typename eastl::enable_if<!is_functor_inplace_allocatable<Functor, SIZE_IN_BYTES>::value>::type>
        {
          public:
            ...
            MUDA_THREAD_ONLY static Functor*& GetFunctorPtrRef(const FunctorStorageType& storage) EA_NOEXCEPT
            {
                return storage.template GetStorageTypeRef<Functor*>();
            }
        ...
        class function_manager final : public function_manager_base<Functor>
        {
          public:
...
#if EASTL_RTTI_ENABLED
            MUDA_THREAD_ONLY static void* GetTypeInfo() EA_NOEXCEPT
            {
                                                  ...
                                                  typename function_base_detail::ManagerOperations ops) EA_NOEXCEPT
            {
                switch(ops)
                {
                    ...
                    case MGROPS_GET_FUNC_PTR: {
                        return static_cast<void*>(Base::GetFunctorPtr(
                            *static_cast<FunctorStorageType*>(to)));
                    }
                    break;
                    default: {
            ...
            /**
				 ...
				 */
            MUDA_THREAD_ONLY static R Invoker(Args... args, const FunctorStorageType& functor)
            {
        ...
        MUDA_THREAD_ONLY function_base_detail() EA_NOEXCEPT  = default;
        MUDA_THREAD_ONLY ~function_base_detail() EA_NOEXCEPT = default;
    };
...
#define EASTL_INTERNAL_FUNCTION_DETAIL_VALID_FUNCTION_ARGS(FUNCTOR, RET, ARGS, MYSELF) \
        ...
        : public function_base_detail<SIZE_IN_BYTES>
    {
      ...
      public:
        MUDA_THREAD_ONLY function_detail() EA_NOEXCEPT = default;
        MUDA_THREAD_ONLY function_detail(std::nullptr_t) EA_NOEXCEPT {}

        MUDA_THREAD_ONLY function_detail(const function_detail& other)
        {
            if(this != &other)
            {
                Copy(other);
        ...
        MUDA_THREAD_ONLY function_detail(function_detail&& other)
        {
            if(this != &other)
            {
        ...
        MUDA_THREAD_ONLY function_detail(Functor functor)
        {
            CreateForwardFunctor(eastl::move(functor));
        }

        MUDA_THREAD_ONLY ~function_detail() EA_NOEXCEPT { Destroy(); }

        MUDA_THREAD_ONLY function_detail& operator=(const function_detail& other)
        {
            if(this != &other)
            {
                Destroy();
        ...
        MUDA_THREAD_ONLY function_detail& operator=(function_detail&& other)
        {
            if(this != &other)
            {
                Destroy();
        ...
        MUDA_THREAD_ONLY function_detail& operator=(std::nullptr_t) EA_NOEXCEPT
        {
            Destroy();
            mMgrFuncPtr    = nullptr;
        ...
        template <typename Functor, typename = EASTL_INTERNAL_FUNCTION_DETAIL_VALID_FUNCTION_ARGS(Functor, R, Args..., function_detail)>
        MUDA_THREAD_ONLY function_detail& operator=(Functor&& functor)
        {
            Destroy();
        ...
        template <typename Functor>
        MUDA_THREAD_ONLY function_detail& operator=(eastl::reference_wrapper<Functor> f) EA_NOEXCEPT
        {
            Destroy();
        ...
        MUDA_THREAD_ONLY void swap(function_detail& other) EA_NOEXCEPT
        {
            if(this == &other)
                return;
        ...
        MUDA_THREAD_ONLY explicit operator bool() const EA_NOEXCEPT
        {
            return HaveManager();
        }

        MUDA_THREAD_ONLY EASTL_FORCE_INLINE R operator()(Args... args) const
        {
            return (*mInvokeFuncPtr)(eastl::forward<Args>(args)..., this->mStorage);
        }

#if EASTL_RTTI_ENABLED
        MUDA_THREAD_ONLY const std::type_info& target_type() const EA_NOEXCEPT
        {
            if(HaveManager())
            {
        ...
        template <typename Functor>
        MUDA_THREAD_ONLY Functor* target() EA_NOEXCEPT
        {
            if(HaveManager() && target_type() == typeid(Functor))
            {
                void* ret = (*mMgrFuncPtr)(static_cast<void*>(&mStorage),
                                           nullptr,
        ...
        template <typename Functor>
        MUDA_THREAD_ONLY const Functor* target() const EA_NOEXCEPT
        {
            if(HaveManager() && target_type() == typeid(Functor))
            {
                void* ret = (*mMgrFuncPtr)(static_cast<void*>(&mStorage),
      ...
      private:
        MUDA_THREAD_ONLY bool HaveManager() const EA_NOEXCEPT
        {
            return (mMgrFuncPtr != nullptr);
        }

        MUDA_THREAD_ONLY void Destroy() EA_NOEXCEPT
        {
            if(HaveManager())
            {
                (void)(*mMgrFuncPtr)(static_cast<void*>(&mStorage),
                                     nullptr,
                                     Base::ManagerOperations::MGROPS_DESTRUCT_FUNCTOR);
            }
        }

        MUDA_THREAD_ONLY void Copy(const function_detail& other)
        {
            if(other.HaveManager())
            {
        ...
        MUDA_THREAD_ONLY void Move(function_detail&& other)
        {
            if(other.HaveManager())
            {
                (void)(*other.mMgrFuncPtr)(static_cast<void*>(&mStorage),
        ...
        template <typename Functor>
        MUDA_THREAD_ONLY void CreateForwardFunctor(Functor&& functor)
        {
      ...
      private:
        ...
        MUDA_THREAD_ONLY static R DefaultInvoker(Args... /*args*/,
