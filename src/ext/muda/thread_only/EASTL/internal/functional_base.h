...
MUDA_THREAD_ONLY inline void swap(T& a, T& b)
	...
	MUDA_THREAD_ONLY EA_CONSTEXPR auto invoke_impl(R C::*func, T&& obj, Args&&... args) EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR((eastl::forward<T>(obj).*func)(eastl::forward<Args>(args)...)))
	...
	MUDA_THREAD_ONLY EA_CONSTEXPR auto invoke_impl(F&& func, Args&&... args) EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(eastl::forward<F>(func)(eastl::forward<Args>(args)...)))
	...
	MUDA_THREAD_ONLY EA_CONSTEXPR auto invoke_impl(M C::*member, T&& obj) EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(eastl::forward<T>(obj).*member))
		-> typename enable_if<
	...
	template <typename F, typename... Args>
	MUDA_THREAD_ONLY EA_CONSTEXPR decltype(auto) invoke(F&& func, Args&&... args) EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(invoke_impl(eastl::forward<F>(func), eastl::forward<Args>(args)...)))
	{
	...
	public:
		...
		MUDA_THREAD_ONLY reference_wrapper(T&) EA_NOEXCEPT;
        MUDA_THREAD_ONLY reference_wrapper(T&&) = delete;
        MUDA_THREAD_ONLY reference_wrapper(const reference_wrapper<T>& x) EA_NOEXCEPT;

		MUDA_THREAD_ONLY reference_wrapper& operator=(const reference_wrapper<T>& x) EA_NOEXCEPT;

		MUDA_THREAD_ONLY operator T&() const EA_NOEXCEPT;
        MUDA_THREAD_ONLY T& get() const EA_NOEXCEPT;
	...
	template <typename R, typename C, typename T, typename... Args>
        MUDA_THREAD_ONLY EA_CONSTEXPR auto invoke_impl(R C::*func, T&& obj, Args&&... args)
	...
	template <typename M, typename C, typename T>
    MUDA_THREAD_ONLY EA_CONSTEXPR auto invoke_impl(M C::*member, T&& obj)
        EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(obj.get().*member))
	   ->  typename enable_if<is_reference_wrapper<eastl::decay_t<T>>::value,
	...
	class binder1st : public unary_function<typename Operation::second_argument_type, typename Operation::result_type>
	{
		...
		public:
            MUDA_THREAD_ONLY binder1st(const Operation& x,
                                       const typename Operation::first_argument_type& y)
				: value(y), op(x) { }

			MUDA_THREAD_ONLY typename Operation::result_type operator()(
    ...
    MUDA_THREAD_ONLY inline binder1st<Operation> bind1st(const Operation& op, const T& x)
	{
	...
	class binder2nd : public unary_function<typename Operation::first_argument_type, typename Operation::result_type>
	{
		...
		public:
            MUDA_THREAD_ONLY binder2nd(const Operation& x,
	...
	template <typename Operation, typename T>
    MUDA_THREAD_ONLY inline binder2nd<Operation> bind2nd(const Operation& op, const T& x)
	{
