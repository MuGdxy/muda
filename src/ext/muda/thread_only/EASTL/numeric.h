...
MUDA_THREAD_ONLY inline T accumulate(InputIterator first, InputIterator last, T init)
{
...
MUDA_THREAD_ONLY inline T accumulate(InputIterator first, InputIterator last, T init, BinaryOperation binary_op)
{
    for(; first != last; ++first)
...
MUDA_THREAD_ONLY inline void iota(ForwardIterator first, ForwardIterator last, T value)
{
    while(first != last)
    {
        *first++ = value;
...
MUDA_THREAD_ONLY inline T inner_product(InputIterator1 first1,
...
MUDA_THREAD_ONLY inline OutputIterator partial_sum(InputIterator  first,
                                                   InputIterator  last,
...
///
template <typename InputIterator, typename OutputIterator>
MUDA_THREAD_ONLY inline OutputIterator adjacent_difference(InputIterator first,
                                                           InputIterator last,
                                                           OutputIterator result)
{
    typedef typename iterator_traits<InputIterator>::value_type value_type;
...
///
template <typename T>
MUDA_THREAD_ONLY constexpr eastl::enable_if_t<eastl::is_arithmetic_v<T> && !eastl::is_same_v<eastl::remove_cv_t<T>, bool>, T> midpoint(
    const T lhs, const T rhs) EA_NOEXCEPT
{
...
///
template <typename T>
MUDA_THREAD_ONLY constexpr eastl::enable_if_t<eastl::is_object_v<T>, T*> midpoint(T* lhs, T* rhs)
{
...
template <class T>
MUDA_THREAD_ONLY T shared_lerp(const T a, const T b, const T t) EA_NOEXCEPT
{
    if((a <= 0 && b >= 0) || (a >= 0 && b <= 0))
    {
        return t * b + (1 - t) * a;
...
///
MUDA_THREAD_ONLY constexpr float lerp(float a, float b, float t) EA_NOEXCEPT
{
    return shared_lerp(a, b, t);
}
MUDA_THREAD_ONLY constexpr double lerp(double a, double b, double t) EA_NOEXCEPT
{
    return shared_lerp(a, b, t);
}
MUDA_THREAD_ONLY constexpr long double lerp(long double a, long double b, long double t) EA_NOEXCEPT
{
    return shared_lerp(a, b, t);
}
#endif
