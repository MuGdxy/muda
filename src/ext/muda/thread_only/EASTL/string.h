...
namespace StdC
{
    ...
    EASTL_EASTDC_API int Vsnprintf(char32_t* EA_RESTRICT       pDestination,
                                   ...
                                   va_list                     arguments);
#if EA_CHAR8_UNIQUE
    EASTL_EASTDC_API int Vsnprintf(char8_t* EA_RESTRICT       pDestination,
                                   size_t                     n,
                                   const char8_t* EA_RESTRICT pFormat,
                                   va_list                    arguments);
#endif
#if defined(EA_WCHAR_UNIQUE) && EA_WCHAR_UNIQUE
    EASTL_EASTDC_API int Vsnprintf(wchar_t* EA_RESTRICT       pDestination,
                                   size_t                     n,
...
inline int Vsnprintf(char16_t* EA_RESTRICT       pDestination,
                     size_t                      n,
                     const char16_t* EA_RESTRICT pFormat,
                     va_list                     arguments)
{
...
inline int Vsnprintf(char32_t* EA_RESTRICT       pDestination,
                     size_t                      n,
                     const char32_t* EA_RESTRICT pFormat,
                     va_list                     arguments)
{
...
#if EA_CHAR8_UNIQUE
inline int Vsnprintf(char8_t* EA_RESTRICT       pDestination,
                     size_t                     n,
                     const char8_t* EA_RESTRICT pFormat,
                     va_list                    arguments)
{
...
#if defined(EA_WCHAR_UNIQUE) && EA_WCHAR_UNIQUE
inline int Vsnprintf(wchar_t* EA_RESTRICT       pDestination,
                     size_t                     n,
                     const wchar_t* EA_RESTRICT pFormat,
                     va_list                    arguments)
{
...
// Masks used to determine if we are in SSO or Heap
  ...
  protected:
    ...
    struct Layout
    {
        ...
        MUDA_THREAD_ONLY Layout() { ResetToSSO(); }  // start as SSO by default
        MUDA_THREAD_ONLY Layout(const Layout& other) { Copy(*this, other); }
        MUDA_THREAD_ONLY Layout(Layout&& other) { Move(*this, other); }
        MUDA_THREAD_ONLY Layout& operator=(const Layout& other)
        {
            Copy(*this, other);
            return *this;
        }
        MUDA_THREAD_ONLY Layout& operator=(Layout&& other)
        {
            Move(*this, other);
            return *this;
        }
        ...
        MUDA_THREAD_ONLY inline bool IsHeap() const EA_NOEXCEPT
        {
            return !!(sso.mRemainingSizeField.mnRemainingSize & kSSOMask);
        }
        MUDA_THREAD_ONLY inline bool IsSSO() const EA_NOEXCEPT
        {
            return !IsHeap();
        }
        MUDA_THREAD_ONLY inline value_type* SSOBufferPtr() EA_NOEXCEPT
        {
            return sso.mData;
        }
        MUDA_THREAD_ONLY inline const value_type* SSOBufferPtr() const EA_NOEXCEPT
        {
            return sso.mData;
        }
        ...
        MUDA_THREAD_ONLY inline size_type GetSSOSize() const EA_NOEXCEPT
        {
#ifdef EA_SYSTEM_BIG_ENDIAN
        ...
        MUDA_THREAD_ONLY inline size_type GetHeapSize() const EA_NOEXCEPT
        {
            return heap.mnSize;
        }
        MUDA_THREAD_ONLY inline size_type GetSize() const EA_NOEXCEPT
        {
            return IsHeap() ? GetHeapSize() : GetSSOSize();
        }

        MUDA_THREAD_ONLY inline void SetSSOSize(size_type size) EA_NOEXCEPT
        {
#ifdef EA_SYSTEM_BIG_ENDIAN
        ...
        MUDA_THREAD_ONLY inline void SetHeapSize(size_type size) EA_NOEXCEPT
        {
            heap.mnSize = size;
        }
        MUDA_THREAD_ONLY inline void SetSize(size_type size) EA_NOEXCEPT
        {
            IsHeap() ? SetHeapSize(size) : SetSSOSize(size);
        }

        MUDA_THREAD_ONLY inline size_type GetRemainingCapacity() const EA_NOEXCEPT
        {
            return size_type(CapacityPtr() - EndPtr());
        }

        MUDA_THREAD_ONLY inline value_type* HeapBeginPtr() EA_NOEXCEPT
        {
            return heap.mpBegin;
        };
        MUDA_THREAD_ONLY inline const value_type* HeapBeginPtr() const EA_NOEXCEPT
        {
            return heap.mpBegin;
        };

        MUDA_THREAD_ONLY inline value_type* SSOBeginPtr() EA_NOEXCEPT
        {
            return sso.mData;
        }
        MUDA_THREAD_ONLY inline const value_type* SSOBeginPtr() const EA_NOEXCEPT
        {
            return sso.mData;
        }

        MUDA_THREAD_ONLY inline value_type* BeginPtr() EA_NOEXCEPT
        {
            return IsHeap() ? HeapBeginPtr() : SSOBeginPtr();
        }
        MUDA_THREAD_ONLY inline const value_type* BeginPtr() const EA_NOEXCEPT
        {
            return IsHeap() ? HeapBeginPtr() : SSOBeginPtr();
        }

        MUDA_THREAD_ONLY inline value_type* HeapEndPtr() EA_NOEXCEPT
        {
            return heap.mpBegin + heap.mnSize;
        }
        MUDA_THREAD_ONLY inline const value_type* HeapEndPtr() const EA_NOEXCEPT
        {
            return heap.mpBegin + heap.mnSize;
        }

        MUDA_THREAD_ONLY inline value_type* SSOEndPtr() EA_NOEXCEPT
        {
            return sso.mData + GetSSOSize();
        }
        MUDA_THREAD_ONLY inline const value_type* SSOEndPtr() const EA_NOEXCEPT
        {
            return sso.mData + GetSSOSize();
        }

        // Points to end of character stream, *ptr == '0'
        MUDA_THREAD_ONLY inline value_type* EndPtr() EA_NOEXCEPT
        {
            return IsHeap() ? HeapEndPtr() : SSOEndPtr();
        }
        MUDA_THREAD_ONLY inline const value_type* EndPtr() const EA_NOEXCEPT
        {
            return IsHeap() ? HeapEndPtr() : SSOEndPtr();
        }

        MUDA_THREAD_ONLY inline value_type* HeapCapacityPtr() EA_NOEXCEPT
        {
            return heap.mpBegin + GetHeapCapacity();
        }
        MUDA_THREAD_ONLY inline const value_type* HeapCapacityPtr() const EA_NOEXCEPT
        {
            return heap.mpBegin + GetHeapCapacity();
        }

        MUDA_THREAD_ONLY inline value_type* SSOCapacityPtr() EA_NOEXCEPT
        {
            return sso.mData + SSOLayout::SSO_CAPACITY;
        }
        MUDA_THREAD_ONLY inline const value_type* SSOCapacityPtr() const EA_NOEXCEPT
        {
            return sso.mData + SSOLayout::SSO_CAPACITY;
        }

        // Points to end of the buffer at the terminating '0', *ptr == '0' <- only true when size() == capacity()
        MUDA_THREAD_ONLY inline value_type* CapacityPtr() EA_NOEXCEPT
        {
            return IsHeap() ? HeapCapacityPtr() : SSOCapacityPtr();
        }
        MUDA_THREAD_ONLY inline const value_type* CapacityPtr() const EA_NOEXCEPT
        {
            return IsHeap() ? HeapCapacityPtr() : SSOCapacityPtr();
        }

        MUDA_THREAD_ONLY inline void SetHeapBeginPtr(value_type* pBegin) EA_NOEXCEPT
        {
            heap.mpBegin = pBegin;
        }

        MUDA_THREAD_ONLY inline void SetHeapCapacity(size_type cap) EA_NOEXCEPT
        {
#ifdef EA_SYSTEM_BIG_ENDIAN
        ...
        MUDA_THREAD_ONLY inline size_type GetHeapCapacity() const EA_NOEXCEPT
        {
#ifdef EA_SYSTEM_BIG_ENDIAN
        ...
        MUDA_THREAD_ONLY inline void Copy(Layout& dst, const Layout& src) EA_NOEXCEPT
        {
            dst.raw = src.raw;
        }
        MUDA_THREAD_ONLY inline void Move(Layout& dst, Layout& src) EA_NOEXCEPT
        {
            eastl::swap(dst.raw, src.raw);
        }
        MUDA_THREAD_ONLY inline void Swap(Layout& a, Layout& b) EA_NOEXCEPT
        {
            eastl::swap(a.raw, b.raw);
        }

        MUDA_THREAD_ONLY inline void ResetToSSO() EA_NOEXCEPT
        {
            *SSOBeginPtr() = 0;
            SetSSOSize(0);
        }
    ...
    MUDA_THREAD_ONLY inline Layout& internalLayout() EA_NOEXCEPT
    {
        return mPair.first();
    }
    MUDA_THREAD_ONLY inline const Layout& internalLayout() const EA_NOEXCEPT
    {
        return mPair.first();
    }
    MUDA_THREAD_ONLY inline allocator_type& internalAllocator() EA_NOEXCEPT
    {
        return mPair.second();
    }
    MUDA_THREAD_ONLY inline const allocator_type& internalAllocator() const EA_NOEXCEPT
    {
        return mPair.second();
    }
  ...
  public:
    ...
    MUDA_THREAD_ONLY basic_string()
        EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(EASTL_BASIC_STRING_DEFAULT_ALLOCATOR));
    MUDA_THREAD_ONLY explicit basic_string(const allocator_type& allocator) EA_NOEXCEPT;
    MUDA_THREAD_ONLY basic_string(const this_type& x, size_type position, size_type n = npos);
    MUDA_THREAD_ONLY                       basic_string(const value_type* p,
                                                        size_type         n,
                                                        const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    MUDA_THREAD_ONLY EASTL_STRING_EXPLICIT basic_string(
        const value_type*     p,
        const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    MUDA_THREAD_ONLY basic_string(size_type  n,
                                  value_type c,
                                  const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    MUDA_THREAD_ONLY basic_string(const this_type& x);
    MUDA_THREAD_ONLY basic_string(const this_type& x, const allocator_type& allocator);
    MUDA_THREAD_ONLY basic_string(const value_type* pBegin,
                                  const value_type* pEnd,
                                  const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    MUDA_THREAD_ONLY basic_string(CtorDoNotInitialize,
                                  size_type n,
                                  const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    MUDA_THREAD_ONLY basic_string(CtorSprintf, const value_type* pFormat, ...);
    MUDA_THREAD_ONLY basic_string(std::initializer_list<value_type> init,
                                  const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);

    MUDA_THREAD_ONLY basic_string(this_type&& x) EA_NOEXCEPT;
    MUDA_THREAD_ONLY basic_string(this_type&& x, const allocator_type& allocator);

    MUDA_THREAD_ONLY explicit basic_string(const view_type& sv,
                                           const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    MUDA_THREAD_ONLY basic_string(const view_type& sv,
                                  size_type        position,
                                  size_type        n,
                                  const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);

    template <typename OtherCharType>
    MUDA_THREAD_ONLY basic_string(CtorConvert,
    ...
    MUDA_THREAD_ONLY basic_string(CtorConvert, const OtherStringType& x);

    MUDA_THREAD_ONLY ~basic_string();

    // Allocator
    MUDA_THREAD_ONLY const allocator_type& get_allocator() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY allocator_type&       get_allocator() EA_NOEXCEPT;
    MUDA_THREAD_ONLY void set_allocator(const allocator_type& allocator);

    // Implicit conversion operator
    MUDA_THREAD_ONLY operator basic_string_view<T>() const EA_NOEXCEPT;

    // Operator=
    MUDA_THREAD_ONLY this_type& operator=(const this_type& x);
    MUDA_THREAD_ONLY this_type& operator=(const value_type* p);
    MUDA_THREAD_ONLY this_type& operator=(value_type c);
    MUDA_THREAD_ONLY this_type& operator=(std::initializer_list<value_type> ilist);
    MUDA_THREAD_ONLY this_type& operator=(view_type v);
    MUDA_THREAD_ONLY this_type& operator=(this_type&& x);  // TODO(c++17): noexcept(allocator_traits<Allocator>::propagate_on_container_move_assignment::value || allocator_traits<Allocator>::is_always_equal::value);

#if EASTL_OPERATOR_EQUALS_OTHER_ENABLED
    MUDA_THREAD_ONLY this_type& operator=(value_type* p)
    {
    ...
    template <typename OtherCharType>
    MUDA_THREAD_ONLY this_type& operator=(const OtherCharType* p);

    template <typename OtherStringType>
    MUDA_THREAD_ONLY this_type& operator=(const OtherStringType& x);
#endif

    MUDA_THREAD_ONLY void swap(this_type& x);  // TODO(c++17): noexcept(allocator_traits<Allocator>::propagate_on_container_swap::value || allocator_traits<Allocator>::is_always_equal::value);

    // Assignment operations
    MUDA_THREAD_ONLY this_type& assign(const this_type& x);
    MUDA_THREAD_ONLY this_type& assign(const this_type& x, size_type position, size_type n = npos);
    MUDA_THREAD_ONLY this_type& assign(const value_type* p, size_type n);
    MUDA_THREAD_ONLY this_type& assign(const value_type* p);
    MUDA_THREAD_ONLY this_type& assign(size_type n, value_type c);
    MUDA_THREAD_ONLY this_type& assign(const value_type* pBegin, const value_type* pEnd);
    MUDA_THREAD_ONLY this_type& assign(this_type&& x);  // TODO(c++17): noexcept(allocator_traits<Allocator>::propagate_on_container_move_assignment::value || allocator_traits<Allocator>::is_always_equal::value);
    MUDA_THREAD_ONLY this_type& assign(std::initializer_list<value_type>);

    template <typename OtherCharType>
    MUDA_THREAD_ONLY this_type& assign_convert(const OtherCharType* p);

    template <typename OtherCharType>
    MUDA_THREAD_ONLY this_type& assign_convert(const OtherCharType* p, size_type n);

    template <typename OtherStringType>
    MUDA_THREAD_ONLY this_type& assign_convert(const OtherStringType& x);

    // Iterators.
    MUDA_THREAD_ONLY iterator begin() EA_NOEXCEPT;  // Expanded in source code as: mpBegin
    MUDA_THREAD_ONLY const_iterator begin() const EA_NOEXCEPT;  // Expanded in source code as: mpBegin
    MUDA_THREAD_ONLY const_iterator cbegin() const EA_NOEXCEPT;

    MUDA_THREAD_ONLY iterator end() EA_NOEXCEPT;  // Expanded in source code as: mpEnd
    MUDA_THREAD_ONLY const_iterator end() const EA_NOEXCEPT;  // Expanded in source code as: mpEnd
    MUDA_THREAD_ONLY const_iterator cend() const EA_NOEXCEPT;

    MUDA_THREAD_ONLY reverse_iterator       rbegin() EA_NOEXCEPT;
    MUDA_THREAD_ONLY const_reverse_iterator rbegin() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY const_reverse_iterator crbegin() const EA_NOEXCEPT;

    MUDA_THREAD_ONLY reverse_iterator       rend() EA_NOEXCEPT;
    MUDA_THREAD_ONLY const_reverse_iterator rend() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY const_reverse_iterator crend() const EA_NOEXCEPT;


    // Size-related functionality
    MUDA_THREAD_ONLY bool      empty() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type size() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type length() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type max_size() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type capacity() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY void      resize(size_type n, value_type c);
    MUDA_THREAD_ONLY void      resize(size_type n);
    MUDA_THREAD_ONLY void      reserve(size_type = 0);
    MUDA_THREAD_ONLY void set_capacity(size_type n = npos);  // Revises the capacity to the user-specified value. Resizes the container to match the capacity if the requested capacity n is less than the current size. If n == npos then the capacity is reallocated (if necessary) such that capacity == size.
    MUDA_THREAD_ONLY void force_size(size_type n);  // Unilaterally moves the string end position (mpEnd) to the given location. Useful for when the user writes into the string via some extenal means such as C strcpy or sprintf. This allows for more efficient use than using resize to achieve this.
    MUDA_THREAD_ONLY void shrink_to_fit();

    // Raw access
    MUDA_THREAD_ONLY const value_type* data() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY value_type*       data() EA_NOEXCEPT;
    MUDA_THREAD_ONLY const value_type* c_str() const EA_NOEXCEPT;

    // Element access
    MUDA_THREAD_ONLY reference       operator[](size_type n);
    MUDA_THREAD_ONLY const_reference operator[](size_type n) const;
    MUDA_THREAD_ONLY reference       at(size_type n);
    MUDA_THREAD_ONLY const_reference at(size_type n) const;
    MUDA_THREAD_ONLY reference       front();
    MUDA_THREAD_ONLY const_reference front() const;
    MUDA_THREAD_ONLY reference       back();
    MUDA_THREAD_ONLY const_reference back() const;
    ...
    MUDA_THREAD_ONLY this_type& append(const this_type& x);
    MUDA_THREAD_ONLY this_type& append(const this_type& x, size_type position, size_type n = npos);
    MUDA_THREAD_ONLY this_type& append(const value_type* p, size_type n);
    MUDA_THREAD_ONLY this_type& append(const value_type* p);
    MUDA_THREAD_ONLY this_type& append(size_type n, value_type c);
    MUDA_THREAD_ONLY this_type& append(const value_type* pBegin, const value_type* pEnd);

    MUDA_THREAD_ONLY this_type& append_sprintf_va_list(const value_type* pFormat,
                                                       va_list arguments);
    MUDA_THREAD_ONLY this_type& append_sprintf(const value_type* pFormat, ...);

    template <typename OtherCharType>
    MUDA_THREAD_ONLY this_type& append_convert(const OtherCharType* p);

    template <typename OtherCharType>
    MUDA_THREAD_ONLY this_type& append_convert(const OtherCharType* p, size_type n);

    template <typename OtherStringType>
    MUDA_THREAD_ONLY this_type& append_convert(const OtherStringType& x);

    MUDA_THREAD_ONLY void push_back(value_type c);
    MUDA_THREAD_ONLY void pop_back();

    // Insertion operations
    MUDA_THREAD_ONLY this_type& insert(size_type position, const this_type& x);
    MUDA_THREAD_ONLY this_type& insert(size_type position, const this_type& x, size_type beg, size_type n);
    MUDA_THREAD_ONLY this_type& insert(size_type position, const value_type* p, size_type n);
    MUDA_THREAD_ONLY this_type& insert(size_type position, const value_type* p);
    MUDA_THREAD_ONLY this_type& insert(size_type position, size_type n, value_type c);
    MUDA_THREAD_ONLY iterator insert(const_iterator p, value_type c);
    MUDA_THREAD_ONLY iterator insert(const_iterator p, size_type n, value_type c);
    MUDA_THREAD_ONLY iterator insert(const_iterator    p,
                                     const value_type* pBegin,
                                     const value_type* pEnd);
    MUDA_THREAD_ONLY iterator insert(const_iterator p, std::initializer_list<value_type>);

    // Erase operations
    MUDA_THREAD_ONLY this_type& erase(size_type position = 0, size_type n = npos);
    MUDA_THREAD_ONLY iterator erase(const_iterator p);
    MUDA_THREAD_ONLY iterator erase(const_iterator pBegin, const_iterator pEnd);
    MUDA_THREAD_ONLY reverse_iterator erase(reverse_iterator position);
    MUDA_THREAD_ONLY reverse_iterator erase(reverse_iterator first, reverse_iterator last);
    MUDA_THREAD_ONLY void clear() EA_NOEXCEPT;

    // Detach memory
    MUDA_THREAD_ONLY pointer detach() EA_NOEXCEPT;

    // Replacement operations
    MUDA_THREAD_ONLY this_type& replace(size_type position, size_type n, const this_type& x);
    MUDA_THREAD_ONLY this_type& replace(size_type        pos1,
                                        size_type        n1,
                                        const this_type& x,
                                        size_type        pos2,
                                        size_type        n2 = npos);
    MUDA_THREAD_ONLY this_type& replace(size_type         position,
                                        size_type         n1,
                                        const value_type* p,
                                        size_type         n2);
    MUDA_THREAD_ONLY this_type& replace(size_type position, size_type n1, const value_type* p);
    MUDA_THREAD_ONLY this_type& replace(size_type position, size_type n1, size_type n2, value_type c);
    MUDA_THREAD_ONLY this_type& replace(const_iterator   first,
                                        const_iterator   last,
                                        const this_type& x);
    MUDA_THREAD_ONLY this_type& replace(const_iterator    first,
                                        const_iterator    last,
    ...
    // Find operations
    MUDA_THREAD_ONLY size_type find(const this_type& x, size_type position = 0) const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type find(const value_type* p, size_type position = 0) const;
    MUDA_THREAD_ONLY size_type find(const value_type* p, size_type position, size_type n) const;
    MUDA_THREAD_ONLY size_type find(value_type c, size_type position = 0) const EA_NOEXCEPT;

    // Reverse find operations
    MUDA_THREAD_ONLY size_type rfind(const this_type& x, size_type position = npos) const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type rfind(const value_type* p, size_type position = npos) const;
    MUDA_THREAD_ONLY size_type rfind(const value_type* p, size_type position, size_type n) const;
    MUDA_THREAD_ONLY size_type rfind(value_type c, size_type position = npos) const EA_NOEXCEPT;

    // Find first-of operations
    MUDA_THREAD_ONLY size_type find_first_of(const this_type& x,
                                             size_type position = 0) const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type find_first_of(const value_type* p,
                                             size_type position = 0) const;
    MUDA_THREAD_ONLY size_type find_first_of(const value_type* p,
                                             size_type         position,
                                             size_type         n) const;
    MUDA_THREAD_ONLY size_type find_first_of(value_type c, size_type position = 0) const EA_NOEXCEPT;

    // Find last-of operations
    MUDA_THREAD_ONLY size_type find_last_of(const this_type& x,
                                            size_type position = npos) const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type find_last_of(const value_type* p,
                                            size_type position = npos) const;
    MUDA_THREAD_ONLY size_type find_last_of(const value_type* p,
                                            size_type         position,
                                            size_type         n) const;
    MUDA_THREAD_ONLY size_type find_last_of(value_type c, size_type position = npos) const EA_NOEXCEPT;

    // Find first not-of operations
    MUDA_THREAD_ONLY size_type find_first_not_of(const this_type& x,
                                                 size_type position = 0) const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type find_first_not_of(const value_type* p,
                                                 size_type position = 0) const;
    MUDA_THREAD_ONLY size_type find_first_not_of(const value_type* p,
                                                 size_type         position,
                                                 size_type         n) const;
    MUDA_THREAD_ONLY size_type find_first_not_of(value_type c, size_type position = 0) const EA_NOEXCEPT;

    // Find last not-of operations
    MUDA_THREAD_ONLY size_type find_last_not_of(const this_type& x,
                                                size_type position = npos) const EA_NOEXCEPT;
    MUDA_THREAD_ONLY size_type find_last_not_of(const value_type* p,
                                                size_type position = npos) const;
    MUDA_THREAD_ONLY size_type find_last_not_of(const value_type* p,
                                                size_type         position,
                                                size_type         n) const;
    MUDA_THREAD_ONLY size_type find_last_not_of(value_type c,
                                                size_type position = npos) const EA_NOEXCEPT;

    // Substring functionality
    MUDA_THREAD_ONLY this_type substr(size_type position = 0, size_type n = npos) const;

    // Comparison operations
    MUDA_THREAD_ONLY int compare(const this_type& x) const EA_NOEXCEPT;
    MUDA_THREAD_ONLY int compare(size_type pos1, size_type n1, const this_type& x) const;
    MUDA_THREAD_ONLY int compare(size_type        pos1,
                                 size_type        n1,
                                 const this_type& x,
                                 size_type        pos2,
                                 size_type        n2) const;
    MUDA_THREAD_ONLY int compare(const value_type* p) const;
    MUDA_THREAD_ONLY int compare(size_type pos1, size_type n1, const value_type* p) const;
    MUDA_THREAD_ONLY int compare(size_type pos1, size_type n1, const value_type* p, size_type n2) const;
    MUDA_THREAD_ONLY static int compare(const value_type* pBegin1,
    ...
    MUDA_THREAD_ONLY int        comparei(const this_type& x) const EA_NOEXCEPT;
    MUDA_THREAD_ONLY int        comparei(const value_type* p) const;
    MUDA_THREAD_ONLY static int comparei(const value_type* pBegin1,
                                         const value_type* pEnd1,
                                         const value_type* pBegin2,
                                         const value_type* pEnd2);

    // Misc functionality, not part of C++ this_type.
    MUDA_THREAD_ONLY void      make_lower();
    MUDA_THREAD_ONLY void      make_upper();
    MUDA_THREAD_ONLY void      ltrim();
    MUDA_THREAD_ONLY void      rtrim();
    MUDA_THREAD_ONLY void      trim();
    MUDA_THREAD_ONLY void      ltrim(const value_type* p);
    MUDA_THREAD_ONLY void      rtrim(const value_type* p);
    MUDA_THREAD_ONLY void      trim(const value_type* p);
    MUDA_THREAD_ONLY this_type left(size_type n) const;
    MUDA_THREAD_ONLY this_type right(size_type n) const;
    MUDA_THREAD_ONLY this_type& sprintf_va_list(const value_type* pFormat, va_list arguments);
    MUDA_THREAD_ONLY this_type& sprintf(const value_type* pFormat, ...);

    MUDA_THREAD_ONLY bool validate() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY int  validate_iterator(const_iterator i) const EA_NOEXCEPT;


  protected:
    // Helper functions for initialization/insertion operations.
    MUDA_THREAD_ONLY value_type* DoAllocate(size_type n);
    MUDA_THREAD_ONLY void        DoFree(value_type* p, size_type n);
    MUDA_THREAD_ONLY size_type   GetNewCapacity(size_type currentCapacity);
    MUDA_THREAD_ONLY size_type   GetNewCapacity(size_type currentCapacity,
                                                size_type minimumGrowSize);
    MUDA_THREAD_ONLY void        AllocateSelf();
    MUDA_THREAD_ONLY void        AllocateSelf(size_type n);
    MUDA_THREAD_ONLY void        DeallocateSelf();
    MUDA_THREAD_ONLY iterator    InsertInternal(const_iterator p, value_type c);
    MUDA_THREAD_ONLY void RangeInitialize(const value_type* pBegin, const value_type* pEnd);
    MUDA_THREAD_ONLY void RangeInitialize(const value_type* pBegin);
    MUDA_THREAD_ONLY void SizeInitialize(size_type n, value_type c);

    MUDA_THREAD_ONLY bool IsSSO() const EA_NOEXCEPT;

    MUDA_THREAD_ONLY void ThrowLengthException() const;
    MUDA_THREAD_ONLY void ThrowRangeException() const;
    MUDA_THREAD_ONLY void ThrowInvalidArgumentException() const;

#if EASTL_OPERATOR_EQUALS_OTHER_ENABLED
    template <typename CharType>
    MUDA_THREAD_ONLY void DoAssignConvert(CharType c, true_type);

    template <typename StringType>
    MUDA_THREAD_ONLY void DoAssignConvert(const StringType& x, false_type);
#endif

    // Replacements for STL template functions.
    MUDA_THREAD_ONLY static const value_type* CharTypeStringFindEnd(
        const value_type* pBegin, const value_type* pEnd, value_type c);
    MUDA_THREAD_ONLY static const value_type* CharTypeStringRFind(
        const value_type* pRBegin, const value_type* pREnd, const value_type c);
    MUDA_THREAD_ONLY static const value_type* CharTypeStringSearch(const value_type* p1Begin,
                                                                   const value_type* p1End,
                                                                   const value_type* p2Begin,
                                                                   const value_type* p2End);
    MUDA_THREAD_ONLY static const value_type* CharTypeStringRSearch(const value_type* p1Begin,
                                                                    const value_type* p1End,
                                                                    const value_type* p2Begin,
                                                                    const value_type* p2End);
    MUDA_THREAD_ONLY static const value_type* CharTypeStringFindFirstOf(
        const value_type* p1Begin,
        const value_type* p1End,
        const value_type* p2Begin,
        const value_type* p2End);
    MUDA_THREAD_ONLY static const value_type* CharTypeStringRFindFirstOf(
        const value_type* p1RBegin,
        const value_type* p1REnd,
        const value_type* p2Begin,
        const value_type* p2End);
    MUDA_THREAD_ONLY static const value_type* CharTypeStringFindFirstNotOf(
        const value_type* p1Begin,
        const value_type* p1End,
        const value_type* p2Begin,
        const value_type* p2End);
    MUDA_THREAD_ONLY static const value_type* CharTypeStringRFindFirstNotOf(
        const value_type* p1RBegin,
        const value_type* p1REnd,
        const value_type* p2Begin,
        const value_type* p2End);
        ...
        if(EASTL_LIKELY(n))
        {
            ...
            if(n <= SSOLayout::SSO_CAPACITY)
            {
                ...
                CharStringUninitializedCopy(
                    pOldBegin, pOldBegin + n, internalLayout().SSOBeginPtr());
            ...
            pointer pNewEnd = CharStringUninitializedCopy(
                internalLayout().BeginPtr(), internalLayout().EndPtr(), pNewBegin);
            *pNewEnd = 0;
                                                               ...
                                                               const value_type* pEnd)
{
    if(pBegin != pEnd)
    {
        ...
        {
            pointer pNewEnd =
                CharStringUninitializedCopy(pBegin, pEnd, internalLayout().EndPtr());
            *pNewEnd = 0;
    ...
    const value_type* pFormat, va_list arguments)
{
    ...
    if(nReturnValue > 0)
    {
        ...
        nReturnValue = eastl::Vsnprintf(internalLayout().BeginPtr() + nInitialSize,
                                        static_cast<size_t>(nReturnValue) + 1,
                                        pFormat,
                                        arguments);
    }
    ...
    nReturnValue = eastl::Vsnprintf(
        internalLayout().EndPtr(), (size_t)nInitialRemainingCapacity + 1, pFormat, arguments);
    ...
    else if(nReturnValue < 0)  // If vsnprintf is non-C99-standard
    {
        ...
        for(; (nReturnValue < 0) && (n < 1000000); n *= 2)
        {
            ...
            nReturnValue           = eastl::Vsnprintf(
                internalLayout().BeginPtr() + nInitialSize, nCapacity + 1, pFormat, arguments);
                                                               ...
                                                               const value_type* pEnd)
{
    ...
    if(n <= internalLayout().GetSize())
    {
        muda::memmove(internalLayout().BeginPtr(), pBegin, (size_t)n * sizeof(value_type));
        erase(internalLayout().BeginPtr() + n, internalLayout().EndPtr());
    }
    else
    {
        muda::memmove(internalLayout().BeginPtr(),
                      pBegin,
...
typename basic_string<T, Allocator>::iterator basic_string<T, Allocator>::insert(
    const_iterator p, size_type n, value_type c)
{
    ...
    if(n)  // If there is anything to insert...
    {
        if(internalLayout().GetRemainingCapacity() >= n)  // If we have enough capacity...
        {
            ...
            if(nElementsAfter >= n)  // If there's enough space for the new chars between the insert position and the end...
            {
                ...
                muda::memmove(const_cast<value_type*>(p) + n,
                              p,
            ...
            else
            {
                ...
                const size_type nOldSize = internalLayout().GetSize();
#endif
                CharStringUninitializedFillN(
                    internalLayout().EndPtr() + 1, n - nElementsAfter - 1, c);
        ...
        else
        {
            ...
            iterator pNewEnd =
                CharStringUninitializedCopy(internalLayout().BeginPtr(), p, pNewBegin);
        ...
        if(bCapacityIsSufficient && !bSourceIsFromSelf)
        {
            ...
            if(nElementsAfter >= n)  // If there are enough characters between insert pos and end
            {
                ...
                muda::memmove(const_cast<value_type*>(p) + n,
                              p,
                              (size_t)((nElementsAfter - n) + 1) * sizeof(value_type));
                muda::memmove(const_cast<value_type*>(p), pBegin, (size_t)(n) * sizeof(value_type));
            }
            else
            {
                pointer pOldEnd = internalLayout().EndPtr();
...
inline typename basic_string<T, Allocator>::iterator basic_string<T, Allocator>::erase(const_iterator p)
{
    ...
    muda::memmove(const_cast<value_type*>(p),
                  p + 1,
    ...
    const_iterator pBegin, const_iterator pEnd)
{
    ...
    if(pBegin != pEnd)
    {
        muda::memmove((char*)const_cast<value_type*>(pBegin),
                      (const char*)pEnd,
...
basic_string<T, Allocator>& basic_string<T, Allocator>::replace(
    size_type pos1, size_type n1, const this_type& x, size_type pos2, size_type n2)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
...
#if EASTL_STRING_OPT_LENGTH_ERRORS
    if(EASTL_UNLIKELY((n2 > max_size())
                      || ((internalLayout().GetSize() - nLength) >= (max_size() - n2))))
        ThrowLengthException();
#endif
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>& basic_string<T, Allocator>::replace(
    const_iterator pBegin, const_iterator pEnd, const this_type& x)
{
    return replace(
        pBegin, pEnd, x.internalLayout().BeginPtr(), x.internalLayout().EndPtr());
}


template <typename T, typename Allocator>
inline basic_string<T, Allocator>& basic_string<T, Allocator>::replace(
    const_iterator pBegin, const_iterator pEnd, const value_type* p, size_type n)
{
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>& basic_string<T, Allocator>::replace(
    const_iterator pBegin, const_iterator pEnd, const value_type* p)
{
...
template <typename T, typename Allocator>
basic_string<T, Allocator>& basic_string<T, Allocator>::replace(
    const_iterator pBegin, const_iterator pEnd, size_type n, value_type c)
{
#if EASTL_ASSERT_ENABLED
    if(EASTL_UNLIKELY((pBegin < internalLayout().BeginPtr())
                      || (pBegin > internalLayout().EndPtr())
                                                                ...
                                                                const value_type* pEnd2)
{
    ...
    if(EASTL_UNLIKELY((pBegin1 < internalLayout().BeginPtr())
                      ...
                      || (pEnd1 > internalLayout().EndPtr()) || (pEnd1 < pBegin1)))
        EASTL_FAIL_MSG("basic_string::replace -- invalid position");
#endif
    ...
    if(nLength1 >= nLength2)  // If we have a non-expanding operation...
    {
        if((pBegin2 > pEnd1) || (pEnd2 <= pBegin1))  // If we have a non-overlapping operation...
            muda::memcpy(const_cast<value_type*>(pBegin1),
                         pBegin2,
                         (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
        else
            muda::memmove(const_cast<value_type*>(pBegin1),
                          pBegin2,
                          (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
    ...
    else  // Else we are expanding.
    {
        if((pBegin2 > pEnd1) || (pEnd2 <= pBegin1))  // If we have a non-overlapping operation...
        {
            ...
            if((pEnd2 <= pBegin1) || (pBegin2 > pEnd1))
                muda::memcmp(const_cast<value_type*>(pBegin1),
                             pBegin2,
        ...
        else  // else we have an overlapping operation.
        {
            ...
            const size_type nNewCapacity =
                GetNewCapacity(nOldCap, (nOldSize + (nLength2 - nLength1)) - nOldCap);

            pointer pNewBegin = DoAllocate(nNewCapacity + 1);

            pointer pNewEnd =
                CharStringUninitializedCopy(internalLayout().BeginPtr(), pBegin1, pNewBegin);
    ...
    const value_type* p, size_type position, size_type n) const
{
    ...
    {
        const value_type* const pTemp = eastl::search(
            internalLayout().BeginPtr() + position, internalLayout().EndPtr(), p, p + n);
    ...
    value_type c, size_type position) const EA_NOEXCEPT
{
    ...
    {
        const const_iterator pResult = eastl::find(
            internalLayout().BeginPtr() + position, internalLayout().EndPtr(), c);
    ...
    if(EASTL_LIKELY(n <= nLength))
    {
        if(EASTL_LIKELY(n))
        {
            ...
            const const_iterator pResult =
                CharTypeStringRSearch(internalLayout().BeginPtr(), pEnd, p, p + n);
    ...
    value_type c, size_type position) const EA_NOEXCEPT
{
    ...
    if(EASTL_LIKELY(nLength))
    {
        const value_type* const pEnd =
            internalLayout().BeginPtr() + eastl::min_alt(nLength - 1, position) + 1;
    ...
    const this_type& x, size_type position) const EA_NOEXCEPT
{
    return find_first_of(
        x.internalLayout().BeginPtr(), position, x.internalLayout().GetSize());
}
    ...
    const value_type* p, size_type position, size_type n) const
{
    ...
    if(EASTL_LIKELY((position < internalLayout().GetSize())))
    {
        ...
        const const_iterator    pResult =
            CharTypeStringFindFirstOf(pBegin, internalLayout().EndPtr(), p, p + n);
    ...
    const value_type* p, size_type position, size_type n) const
{
    ...
    if(EASTL_LIKELY(nLength))
    {
        const value_type* const pEnd =
            internalLayout().BeginPtr() + eastl::min_alt(nLength - 1, position) + 1;
        const value_type* const pResult =
            CharTypeStringRFindFirstOf(pEnd, internalLayout().BeginPtr(), p, p + n);
    ...
    const value_type* p, size_type position, size_type n) const
{
    if(EASTL_LIKELY(position <= internalLayout().GetSize()))
    {
        const const_iterator pResult = CharTypeStringFindFirstNotOf(
            internalLayout().BeginPtr() + position, internalLayout().EndPtr(), p, p + n);
    ...
    value_type c, size_type position) const EA_NOEXCEPT
{
    if(EASTL_LIKELY(position <= internalLayout().GetSize()))
    {
        ...
        const const_iterator pResult = CharTypeStringFindFirstNotOf(
            internalLayout().BeginPtr() + position, internalLayout().EndPtr(), &c, &c + 1);
...
{
    return compare(internalLayout().BeginPtr(),
...
template <typename T, typename Allocator>
inline int basic_string<T, Allocator>::compare(
    size_type pos1, size_type n1, const this_type& x, size_type pos2, size_type n2) const
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if(EASTL_UNLIKELY(
...
{
    return comparei(internalLayout().BeginPtr(),
                    internalLayout().EndPtr(),
                    x.internalLayout().BeginPtr(),
                    x.internalLayout().EndPtr());
}
    ...
    const_iterator p, value_type c)
{
    ...
    {
        const size_type nSavedSize = internalLayout().GetSize();
        muda::memmove(const_cast<value_type*>(p) + 1,
                      p,
                      (size_t)(internalLayout().EndPtr() - p) * sizeof(value_type));
    ...
    size_type currentCapacity, size_type minimumGrowSize)
{
    ...
    const size_type nNewCapacity =
        eastl::max_alt(currentCapacity + minimumGrowSize, currentCapacity * 2);
...
// iterator operators
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline bool operator==(const typename basic_string<T, Allocator>::reverse_iterator& r1,
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline bool operator!=(
...
// Operator +
template <typename T, typename Allocator>
MUDA_THREAD_ONLY basic_string<T, Allocator> operator+(const basic_string<T, Allocator>& a,
                                     const basic_string<T, Allocator>& b)
{
    ...
    basic_string<T, Allocator> result(
        cDNI, a.size() + b.size(), const_cast<basic_string<T, Allocator>&>(a).get_allocator());  // Note that we choose to assign a's allocator.
    result.append(a);
    result.append(b);
    return result;
}
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY basic_string<T, Allocator> operator+(
                                     ...
                                     const basic_string<T, Allocator>& b)
{
    ...
    basic_string<T, Allocator> result(
        cDNI, n + b.size(), const_cast<basic_string<T, Allocator>&>(b).get_allocator());
    result.append(p, p + n);
    result.append(b);
    return result;
}
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY basic_string<T, Allocator> operator+(typename basic_string<T, Allocator>::value_type c,
                                     const basic_string<T, Allocator>& b)
{
    typedef typename basic_string<T, Allocator>::CtorDoNotInitialize CtorDoNotInitialize;
    CtorDoNotInitialize cDNI;  // GCC 2.x forces us to declare a named temporary like this.
    basic_string<T, Allocator> result(
        cDNI, 1 + b.size(), const_cast<basic_string<T, Allocator>&>(b).get_allocator());
    result.push_back(c);
    result.append(b);
    return result;
}
                                     ...
                                     const typename basic_string<T, Allocator>::value_type* p)
{
    ...
    const typename basic_string<T, Allocator>::size_type n =
        (typename basic_string<T, Allocator>::size_type)CharStrlen(p);
    basic_string<T, Allocator> result(
        cDNI, a.size() + n, const_cast<basic_string<T, Allocator>&>(a).get_allocator());
    result.append(a);
    result.append(p, p + n);
    return result;
}
                                     ...
                                     typename basic_string<T, Allocator>::value_type c)
{
    ...
    CtorDoNotInitialize cDNI;  // GCC 2.x forces us to declare a named temporary like this.
    basic_string<T, Allocator> result(
        cDNI, a.size() + 1, const_cast<basic_string<T, Allocator>&>(a).get_allocator());
    result.append(a);
    result.push_back(c);
    return result;
}
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY basic_string<T, Allocator> operator+(basic_string<T, Allocator>&& a,
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline bool basic_string<T, Allocator>::validate() const EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline int basic_string<T, Allocator>::validate_iterator(const_iterator i) const EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline bool operator==(const basic_string<T, Allocator>& a,
                       const basic_string<T, Allocator>& b)
{
    return ((a.size() == b.size())
            && (muda::memcmp(a.data(),
                             b.data(),
                             (size_t)a.size() * sizeof(typename basic_string<T, Allocator>::value_type))
                == 0));
}

#if !defined(EA_COMPILER_HAS_THREE_WAY_COMPARISON)
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline bool operator==(const typename basic_string<T, Allocator>::value_type* p,
                       const basic_string<T, Allocator>&                      b)
{
    typedef typename basic_string<T, Allocator>::size_type size_type;
    const size_type n = (size_type)CharStrlen(p);
    return ((n == b.size()) && (muda::memcmp(p, b.data(), (size_t)n * sizeof(*p)) == 0));
}
#endif
                       ...
                       const typename basic_string<T, Allocator>::value_type* p)
{
    typedef typename basic_string<T, Allocator>::size_type size_type;
    const size_type n = (size_type)CharStrlen(p);
    return ((a.size() == n) && (muda::memcmp(a.data(), p, (size_t)n * sizeof(*p)) == 0));
}

#if defined(EA_COMPILER_HAS_THREE_WAY_COMPARISON)
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline auto operator<=>(const basic_string<T, Allocator>& a,
                        const basic_string<T, Allocator>& b)
{
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline auto operator<=>(const basic_string<T, Allocator>& a,
                        const typename basic_string<T, Allocator>::view_type v)
{
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline bool operator==(const typename basic_string<T, Allocator>::view_type v,
                       const basic_string<T, Allocator>&                    b)
{
...
template <typename T, typename Allocator>
MUDA_THREAD_ONLY inline void swap(basic_string<T, Allocator>& a,
                                  basic_string<T, Allocator>& b)
{
    a.swap(b);
}
...
struct hash<string>
{
    MUDA_THREAD_ONLY size_t operator()(const string& x) const
    {
...
{
    MUDA_THREAD_ONLY size_t operator()(const u8string& x) const
    {
...
{
    MUDA_THREAD_ONLY size_t operator()(const string16& x) const
    {
...
{
    MUDA_THREAD_ONLY size_t operator()(const string32& x) const
    {
...
struct hash<wstring>
{
    MUDA_THREAD_ONLY size_t operator()(const wstring& x) const
    {
...
///
MUDA_THREAD_ONLY inline string to_string(int value)
{
    return string(string::CtorSprintf(), "%d", value);
}
MUDA_THREAD_ONLY inline string to_string(long value)
{
    return string(string::CtorSprintf(), "%ld", value);
}
MUDA_THREAD_ONLY inline string to_string(long long value)
{
    return string(string::CtorSprintf(), "%lld", value);
}
MUDA_THREAD_ONLY inline string to_string(unsigned value)
{
    return string(string::CtorSprintf(), "%u", value);
}
MUDA_THREAD_ONLY inline string to_string(unsigned long value)
{
    return string(string::CtorSprintf(), "%lu", value);
}
MUDA_THREAD_ONLY inline string to_string(unsigned long long value)
{
    return string(string::CtorSprintf(), "%llu", value);
}
MUDA_THREAD_ONLY inline string to_string(float value)
{
    return string(string::CtorSprintf(), "%f", value);
}
MUDA_THREAD_ONLY inline string to_string(double value)
{
    return string(string::CtorSprintf(), "%f", value);
}
MUDA_THREAD_ONLY inline string to_string(long double value)
{
...
///
MUDA_THREAD_ONLY inline wstring to_wstring(int value)
{
    return wstring(wstring::CtorSprintf(), L"%d", value);
}
MUDA_THREAD_ONLY inline wstring to_wstring(long value)
{
    return wstring(wstring::CtorSprintf(), L"%ld", value);
}
MUDA_THREAD_ONLY inline wstring to_wstring(long long value)
{
    return wstring(wstring::CtorSprintf(), L"%lld", value);
}
MUDA_THREAD_ONLY inline wstring to_wstring(unsigned value)
{
    return wstring(wstring::CtorSprintf(), L"%u", value);
}
MUDA_THREAD_ONLY inline wstring to_wstring(unsigned long value)
{
    return wstring(wstring::CtorSprintf(), L"%lu", value);
}
MUDA_THREAD_ONLY inline wstring to_wstring(unsigned long long value)
{
    return wstring(wstring::CtorSprintf(), L"%llu", value);
}
MUDA_THREAD_ONLY inline wstring to_wstring(float value)
{
    return wstring(wstring::CtorSprintf(), L"%f", value);
}
MUDA_THREAD_ONLY inline wstring to_wstring(double value)
{
    return wstring(wstring::CtorSprintf(), L"%f", value);
}
MUDA_THREAD_ONLY inline wstring to_wstring(long double value)
{
...
inline namespace literals
{
    ...
    {
        MUDA_THREAD_ONLY inline string operator"" s(const char* str, size_t len) EA_NOEXCEPT
        {
            return {str, string::size_type(len)};
        }
        MUDA_THREAD_ONLY inline u16string operator"" s(const char16_t* str, size_t len) EA_NOEXCEPT
        {
            return {str, u16string::size_type(len)};
        }
        MUDA_THREAD_ONLY inline u32string operator"" s(const char32_t* str, size_t len) EA_NOEXCEPT
        {
            return {str, u32string::size_type(len)};
        }
        MUDA_THREAD_ONLY inline wstring operator"" s(const wchar_t* str, size_t len) EA_NOEXCEPT
        {
...
MUDA_THREAD_ONLY typename basic_string<CharT, Allocator>::size_type erase(
    basic_string<CharT, Allocator>& c, const U& value)
{
...
MUDA_THREAD_ONLY typename basic_string<CharT, Allocator>::size_type erase_if(
    basic_string<CharT, Allocator>& c, Predicate predicate)
{
