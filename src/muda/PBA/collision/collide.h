...
template <typename T = float, int MAX_QUEUE_SIZE = -1>
struct ticcd
{
    using Scalar  = T;
    ...
    struct NumCCD
    {
        ...
        MUDA_GENERIC uint64_t denominator() const { return details::pow2(denom_power); }
    ...
    MUDA_GENERIC static bool edgeEdgeCCD(const Vector3& a0_start,
    ...
    MUDA_GENERIC static bool edge_edge_interval_root_finder_BFS(
        const Vector3& a0s,
        const Vector3& a1s,
        const Vector3& b0s,
        const Vector3& b1s,
    ...
    MUDA_GENERIC static bool interval_root_finder_BFS(const Vector3& a0s,
                                                      const Vector3& a1s,
                                                      const Vector3& b0s,
                                                      const Vector3& b1s,
                                                      const Vector3& a0e,
    ...
    using IStack = priority_queue<Interval3Pair, vector<Interval3Pair>, Compare>;
                                                      ...
                                                      Scalar& output_tolerance)
    {
        ...
        while(!istack.empty())
        {
            ...
            {
                if constexpr(debugTiccd)
                    muda_kernel_printf("ticcd istack full just return");
                return true;
            }
    ...
    MUDA_GENERIC static int find_next_split(const Array3& widths, const Array3& tols)
    {
        ...
        Array3 tmp =
            (widths > tols).select(widths / tols, -std::numeric_limits<Scalar>::infinity());
        int max_index;
