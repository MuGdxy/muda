...
class FieldEntryViewerBase : public RWViewer
{
  public:
...
class CFieldEntryViewBase : public RWViewer
{
  public:
    MUDA_GENERIC CFieldEntryViewBase() {}

    MUDA_GENERIC CFieldEntryViewBase(const std::byte*          buffer,
                                     const FieldEntryBaseData& info,
                                     details::StringPointer    name_ptr)
        : m_buffer(buffer)
        , m_info(info)
