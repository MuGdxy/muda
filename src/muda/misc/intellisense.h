...
#pragma region cooperative_groups_intellisence
...
#define _CG_THREAD_SCOPE(scope)                                                \
    _CG_STATIC_CONST_DECL cuda::thread_scope thread_scope = scope;
#else
#define _CG_THREAD_SCOPE(scope)
#endif
...
class thread_group
{
  protected:
    ...
    friend _CG_QUALIFIER thread_group tiled_partition(const thread_group& parent,
                                                      unsigned int tilesz);
  ...
  public:
    ...
    _CG_QUALIFIER unsigned int get_type() const { return _data.group.type; }
};

template <unsigned int TyId>
struct thread_group_base : public thread_group
{
    _CG_QUALIFIER thread_group_base()
        : thread_group(TyId)
    {
    }
...
class multi_grid_group : public thread_group_base<details::multi_grid_group_id>
{
  ...
  public:
    ...
    _CG_QUALIFIER unsigned long long size() const { return num_threads(); }
...
_CG_QUALIFIER
#endif
    _CG_DEPRECATED multi_grid_group
    this_multi_grid()
{
...
class grid_group : public thread_group_base<details::grid_group_id>
{
  ...
  public:
    ...
    _CG_QUALIFIER bool is_valid() const { return (_data.grid.gridWs != NULL); }
    ...
    _CG_STATIC_QUALIFIER dim3 group_dim() { return details::grid::grid_dim(); }
...
_CG_CONSTEXPR_QUALIFIER unsigned int scratch_collectives_memory_size(unsigned int communication_size,
...
_CG_CONSTEXPR_QUALIFIER unsigned int scratch_size_needed(unsigned int communication_size,
                                                         unsigned int max_block_size)
{
    return scratch_sync_memory_size(max_block_size)
...
class thread_block : public thread_group_base<details::thread_block_id>
{
    ...
    friend _CG_QUALIFIER thread_group tiled_partition(const thread_group& parent,
                                                      unsigned int tilesz);
    friend _CG_QUALIFIER thread_group tiled_partition(const thread_block& parent,
                                                      unsigned int tilesz);
    ...
    _CG_QUALIFIER thread_block(experimental::block_tile_memory<TileCommunicationSize, MaxBlockSize>& scratch)
        ...
        , communication_size(TileCommunicationSize)
        , max_block_size(MaxBlockSize)
    {
        if(thread_rank() < details::scratch_sync_memory_size(MaxBlockSize)
                               / sizeof(details::barrier_t))
        {
            details::barrier_t* barriers =
                reinterpret_cast<details::barrier_t*>(&tile_memory->memory);
  ...
  public:
    ...
    _CG_STATIC_QUALIFIER void sync() { details::cta::sync(); }

    _CG_STATIC_QUALIFIER unsigned int size() { return details::cta::size(); }
    ...
    _CG_STATIC_QUALIFIER dim3 group_dim() { return details::cta::block_dim(); }
...
class coalesced_group : public thread_group_base<details::coalesced_group_id>
{
  private:
    ...
    friend _CG_QUALIFIER thread_group tiled_partition(const thread_group& parent,
                                                      unsigned int tilesz);
    friend _CG_QUALIFIER coalesced_group tiled_partition(const coalesced_group& parent,
                                                         unsigned int tilesz);
  ...
  public:
    ...
    _CG_QUALIFIER unsigned int size() const { return num_threads(); }
    ...
    _CG_QUALIFIER void sync() const { __syncwarp(_data.coalesced.mask); }
    ...
    {
        unsigned int lane =
            (srcRank == 0) ? __ffs(_data.coalesced.mask) - 1 :
            (size() == 32) ? srcRank :
                             __fns(_data.coalesced.mask, 0, (srcRank + 1));
    ...
    _CG_QUALIFIER TyIntegral shfl(TyIntegral var, unsigned int src_rank) const
    {
        ...
        unsigned int lane =
            (src_rank == 0) ? __ffs(_data.coalesced.mask) - 1 :
            (size() == 32)  ? src_rank :
                              __fns(_data.coalesced.mask, 0, (src_rank + 1));
    ...
    {
        return (__ballot_sync(_data.coalesced.mask, predicate)
                == _data.coalesced.mask);
    }
...
{
template <unsigned int Size>
struct verify_thread_block_tile_size;
template <>
struct verify_thread_block_tile_size<32>
{
    typedef void OK;
};
template <>
struct verify_thread_block_tile_size<16>
{
    typedef void OK;
};
template <>
struct verify_thread_block_tile_size<8>
{
    typedef void OK;
};
template <>
struct verify_thread_block_tile_size<4>
{
    typedef void OK;
};
template <>
struct verify_thread_block_tile_size<2>
{
    typedef void OK;
};
template <>
struct verify_thread_block_tile_size<1>
{
    typedef void OK;
};
...
#ifdef _CG_CPP11_FEATURES
template <unsigned int Size>
using _is_power_of_2 =
    _CG_STL_NAMESPACE::integral_constant<bool, (Size & (Size - 1)) == 0>;
...
class __static_size_tile_base
{
  ...
  public:
    ...
    _CG_STATIC_CONSTEXPR_QUALIFIER unsigned int size() { return num_threads(); }
};
...
class __static_size_thread_block_tile_base : public __static_size_tile_base<Size>
{
...
#ifdef _CG_CPP11_FEATURES
    static_assert(details::_is_valid_single_warp_tile<Size>::value,
  ...
  public:
    ...
    _CG_STATIC_QUALIFIER void sync() { __syncwarp(build_mask()); }
      ...
      public thread_group_base<details::coalesced_group_id>
{
    ...
    friend class __single_warp_thread_block_tile;
    friend class details::_coalesced_group_data_access;

    typedef __static_size_thread_block_tile_base<numThreads> staticSizeBaseT;

  protected:
    _CG_QUALIFIER __single_warp_thread_block_tile(unsigned int meta_group_rank,
...
TyVal _CG_QUALIFIER multi_warp_collectives_helper(const GroupT& group,
...
{
    static_assert(details::_is_valid_multi_warp_tile<Size>::value,
                  "Size must be one of 64/128/256/512");

    template <typename TyVal, typename GroupT, typename WarpLambda, typename InterWarpLambda>
    friend TyVal details::multi_warp_collectives_helper(const GroupT& group,
                                                        WarpLambda warp_lambda,
                                                        InterWarpLambda inter_warp_lambda);
    template <unsigned int OtherSize>
    friend class __static_size_multi_warp_tile_base;
    using WarpType =
        details::internal_thread_block_tile<32, __static_size_multi_warp_tile_base<Size>>;
  ...
  protected:
    ...
    _CG_QUALIFIER __static_size_multi_warp_tile_base(const GroupT& g)
        ...
        , communication_size(g.communication_size)
        , max_block_size(g.max_block_size)
    {
    }
  ...
  private:
    ...
    _CG_QUALIFIER T* get_scratch_location(unsigned int warp_id) const
    {
        ...
        unsigned int scratch_id =
            (details::cta::thread_rank() - thread_rank()) / 32 + warp_id;
        return reinterpret_cast<T*>(
            &tile_memory->memory[sync_mem_size + scratch_id * communication_size]);
    }
    ...
    _CG_QUALIFIER TyVal shfl_iterative_impl(TyVal val, unsigned int src) const
    {
        ...
        details::copy_channel<numWarps> broadcast_channel{
            get_scratch_location<char>(0), get_sync_location(), (size_t)communication_size * numWarps};

        if(warp.meta_group_rank() == src / 32)
        {
            val = warp.shfl(val, src);
            broadcast_channel.template send_value<TyVal, 32, decltype(broadcast_channel)::send_many_to_many>(
    ...
    _CG_QUALIFIER TyVal collectives_scheme_impl(const WarpLambda& warp_lambda,
                                                const InterWarpLambda& inter_warp_lambda) const
    {
        ...
        {
            auto subwarp =
                details::tiled_partition_internal<numWarps, decltype(warp)>();
            if(subwarp.meta_group_rank() == 0)
            {
                TyVal* thread_scratch_location =
                    get_scratch_location<TyVal>(subwarp.thread_rank());
                inter_warp_lambda(subwarp, thread_scratch_location);
            }
            warp.sync();
            details::sync_warps_release(sync_location,
                                        warp.thread_rank() == 0,
                                        details::cta::thread_rank(),
                                        numWarps);
        }
    ...
    template <typename TyVal, typename WarpLambda, typename InterWarpLambda>
    _CG_QUALIFIER TyVal collectives_scheme_iterative_impl(const WarpLambda& warp_lambda,
                                                          const InterWarpLambda& inter_warp_lambda) const
    {
        auto warp = details::tiled_partition_internal<32, ThisType>();
        details::barrier_t*             sync_location = get_sync_location();
        details::copy_channel<numWarps> final_result_channel{
            get_scratch_location<char>(0), sync_location, (size_t)communication_size * numWarps};
        ...
        if(warp.meta_group_rank() == 0)
        {
            ...
            partial_results_channel.template receive_value<TyVal>(
                warp_result, warp.thread_rank() == 0, subwarp.thread_rank() != 0);
            ...
            final_result_channel.template send_value<TyVal, 32, decltype(final_result_channel)::send_many_to_many>(
                warp_result, warp.thread_rank(), details::cta::thread_rank() / 32);
        }
        else
        {
            details::copy_channel<numWarps> partial_results_channel{
                get_scratch_location<char>(), sync_location, (size_t)communication_size};
            ...
            final_result_channel.template receive_value<TyVal>(warp_result,
                                                               warp.thread_rank() == 0);
        }
        sync();
        return warp_result;
    ...
    template <typename TyVal, typename WarpLambda, typename InterWarpLambda>
    _CG_QUALIFIER TyVal collectives_scheme(const WarpLambda& warp_lambda,
                                           const InterWarpLambda& inter_warp_lambda) const
    {
        if(sizeof(TyVal) > communication_size)
        {
            return collectives_scheme_iterative_impl<TyVal, WarpLambda, InterWarpLambda>(
                warp_lambda, inter_warp_lambda);
        }
        else
        {
            return collectives_scheme_impl<TyVal, WarpLambda, InterWarpLambda>(
                warp_lambda, inter_warp_lambda);
        }
    }
  ...
  public:
    ...
    _CG_QUALIFIER int any(int predicate) const
    {
        ...
        auto inter_warp_lambda =
            [](details::internal_thread_block_tile<numWarps, WarpType>& subwarp,
               int* thread_scratch_location)
        {
            *thread_scratch_location =
                __any_sync(0xFFFFFFFFU >> (32 - numWarps), *thread_scratch_location);
        };
    ...
    _CG_QUALIFIER int all(int predicate) const
    {
        ...
        {
            *thread_scratch_location =
                __all_sync(0xFFFFFFFFU >> (32 - numWarps), *thread_scratch_location);
        };
        return collectives_scheme<int>(warp_lambda, inter_warp_lambda);
    }
};
...
template <unsigned int Size>
class __multi_warp_thread_block_tile<Size, void>
    : public __static_size_multi_warp_tile_base<Size>
{
  ...
  protected:
    ...
    _CG_QUALIFIER __multi_warp_thread_block_tile(
        const __multi_warp_thread_block_tile<OtherSize, ParentT>& g)
        : __static_size_multi_warp_tile_base<Size>(g)
        , metaGroupRank(g.meta_group_rank())
        , metaGroupSize(g.meta_group_size())
    {
    }

  public:
    _CG_QUALIFIER unsigned int meta_group_rank() const { return metaGroupRank; }

    _CG_QUALIFIER unsigned int meta_group_size() const { return metaGroupSize; }
};
#endif
...
template <unsigned int Size, typename ParentT>
class thread_block_tile_impl<Size, ParentT, false>
    : public __single_warp_thread_block_tile<Size, ParentT>
{
  protected:
    template <unsigned int OtherSize, typename OtherParentT, bool OtherIsMultiWarp>
    _CG_QUALIFIER thread_block_tile_impl(
        const thread_block_tile_impl<OtherSize, OtherParentT, OtherIsMultiWarp>& g)
        : __single_warp_thread_block_tile<Size, ParentT>(g.meta_group_rank(),
                                                         g.meta_group_size())
    {
    }
...
class thread_block_tile_impl<Size, ParentT, true>
    : public __multi_warp_thread_block_tile<Size, ParentT>
{
  protected:
    template <typename GroupT>
...
template <unsigned int Size, typename ParentT>
class thread_block_tile
    : public details::thread_block_tile_impl<Size, ParentT, details::_is_multi_warp<Size>::value>
{
...
class thread_block_tile<Size, void>
    : public details::thread_block_tile_impl<Size, void, details::_is_multi_warp<Size>::value>
{
    template <unsigned int, typename ParentT>
...
_CG_QUALIFIER thread_block_tile<Size, ParentT> tiled_partition(const ParentT& g)
{
    ...
    static_assert(details::_is_single_warp<Size>::value,
                  "_CG_ABI_EXPERIMENTAL needs to be defined"
...
#ifdef _CG_CPP11_FEATURES
    static_assert(details::_is_single_warp<Size>::value,
                  "Tiled partition with Size > 32 is supported only by"
...
_CG_QUALIFIER void thread_group::sync() const
{
    switch(_data.group.type)
    {
        ...
        case details::grid_group_id:
            ...
            break;
#if defined(_CG_HAS_MULTI_GRID_GROUP) && defined(_CG_CPP11_FEATURES)           \
    && defined(_CG_ABI_EXPERIMENTAL)
...
_CG_QUALIFIER unsigned long long thread_group::size() const
{
    ...
    && defined(_CG_ABI_EXPERIMENTAL)
        case details::multi_grid_group_id:
            size = cooperative_groups::group_size(
                *static_cast<const multi_grid_group*>(this));
            break;
#endif
...
_CG_QUALIFIER unsigned long long thread_group::thread_rank() const
{
    ...
    {
        case details::coalesced_group_id:
            rank = cooperative_groups::thread_rank(
                *static_cast<const coalesced_group*>(this));
            break;
        case details::thread_block_id:
