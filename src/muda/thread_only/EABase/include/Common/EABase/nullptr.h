...
class nullptr_t
{
  public:
    ...
    MUDA_THREAD_ONLY operator T*() const
    {
        return 0;
    }

    template <class C, class T>  // When tested as a member pointer, acts as 0.
    MUDA_THREAD_ONLY operator T C::*() const
    {
        return 0;
    }

    typedef void* (nullptr_t::*bool_)() const;
    MUDA_THREAD_ONLY operator bool_() const  // An rvalue of type std::nullptr_t can be converted to an rvalue of type bool; the resulting value is false.
    {
        return false;
  ...
  private:
    MUDA_THREAD_ONLY void operator&() const;  // Address cannot be taken.
};

MUDA_THREAD_ONLY inline nullptr_t nullptr_get()
{
...
template <class T>
MUDA_THREAD_ONLY inline bool operator==(T* p, const std::nullptr_t)
{
    return p == 0;
}

template <class T>
MUDA_THREAD_ONLY inline bool operator==(const std::nullptr_t, T* p)
{
    return p == 0;
}

template <class T, class U>
MUDA_THREAD_ONLY inline bool operator==(T U::*p, const std::nullptr_t)
{
    return p == 0;
}

template <class T, class U>
MUDA_THREAD_ONLY inline bool operator==(const std::nullptr_t, T U::*p)
{
    return p == 0;
}

MUDA_THREAD_ONLY inline bool operator==(const std::nullptr_t, const std::nullptr_t)
{
    return true;
}
