...
struct move_and_copy_helper
{
    ...
    MUDA_THREAD_ONLY static OutputIterator move_or_copy(InputIterator  first,
                                                        InputIterator  last,
...
//{
//    template <typename T>
//    MUDA_THREAD_ONLY static T* move_or_copy(const T* first, const T* last, T* result)
//    {
...
namespace internal
{
    ...
    using is_contiguous_iterator_helper =
        eastl::is_same<IC, EASTL_ITC_NS::contiguous_iterator_tag>;
#else
    template <typename IC>
    ...
    struct can_be_memmoved_helper
    {
        ...
        static constexpr bool value =
            ...
            && (eastl::is_pointer<InputIterator>::value
                || is_contiguous_iterator_helper<IIC>::value)
            && (eastl::is_pointer<OutputIterator>::value
                || is_contiguous_iterator_helper<OIC>::value);
    };
}  // namespace internal

template <bool isMove, typename InputIterator, typename OutputIterator>
MUDA_THREAD_ONLY inline OutputIterator move_and_copy_chooser(InputIterator first,
                                                             InputIterator last,
                                                             OutputIterator result)
{
    ...
    const bool canBeMemmoved =
        internal::can_be_memmoved_helper<InputIterator, OutputIterator>::value;

    return eastl::move_and_copy_helper<IIC, isMove, canBeMemmoved>::move_or_copy(
        first, last, result);  // Need to chose based on the input iterator tag and not the output iterator tag, because containers accept input ranges of iterator types different than self.
}
...
template <bool isMove, typename InputIterator, typename OutputIterator>
MUDA_THREAD_ONLY inline OutputIterator move_and_copy_unwrapper(InputIterator first,
                                                               InputIterator last,
                                                               OutputIterator result)
{
    return OutputIterator(
        eastl::move_and_copy_chooser<isMove>(eastl::unwrap_iterator(first),
                                             eastl::unwrap_iterator(last),
                                             eastl::unwrap_iterator(result)));  // Have to convert to OutputIterator because unwrap_iterator(result) could be a T*
}
...
template <typename InputIterator, typename OutputIterator>
MUDA_THREAD_ONLY inline OutputIterator move(InputIterator first, InputIterator last, OutputIterator result)
{
...
///
template <typename InputIterator, typename OutputIterator>
MUDA_THREAD_ONLY inline OutputIterator copy(InputIterator first, InputIterator last, OutputIterator result)
{
