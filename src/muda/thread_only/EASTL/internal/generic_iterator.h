...
class generic_iterator
{
  ...
  public:
    ...
    MUDA_THREAD_ONLY generic_iterator()
        : mIterator(iterator_type())
    {
    }

    MUDA_THREAD_ONLY explicit generic_iterator(const iterator_type& x)
        : mIterator(x)
    {
    }

    MUDA_THREAD_ONLY this_type& operator=(const iterator_type& x)
    {
        mIterator = x;
        return *this;
    }
    ...
    MUDA_THREAD_ONLY generic_iterator(const generic_iterator<Iterator2, Container>& x)
        : mIterator(x.base())
    {
    }

    MUDA_THREAD_ONLY reference operator*() const { return *mIterator; }

    MUDA_THREAD_ONLY pointer operator->() const { return mIterator; }

    MUDA_THREAD_ONLY this_type& operator++()
    {
        ++mIterator;
        return *this;
    }

    MUDA_THREAD_ONLY this_type operator++(int)
    {
    ...
    MUDA_THREAD_ONLY reference operator[](const difference_type& n) const
    {
        return mIterator[n];
    }

    MUDA_THREAD_ONLY this_type& operator+=(const difference_type& n)
    {
        mIterator += n;
        return *this;
    }

    MUDA_THREAD_ONLY this_type operator+(const difference_type& n) const
    {
    ...
    MUDA_THREAD_ONLY const iterator_type& base() const { return mIterator; }

  private:
    // Unwrapping interface, not part of the public API.
    MUDA_THREAD_ONLY const iterator_type& unwrap() const { return mIterator; }
...
MUDA_THREAD_ONLY inline bool operator==(const generic_iterator<IteratorL, Container>& lhs,
...
MUDA_THREAD_ONLY inline bool operator==(const generic_iterator<Iterator, Container>& lhs,
...
template <typename IteratorL, typename IteratorR, typename Container>
MUDA_THREAD_ONLY inline typename generic_iterator<IteratorL, Container>::difference_type operator-(
...
template <typename Iterator, typename Container>
MUDA_THREAD_ONLY inline generic_iterator<Iterator, Container> operator+(
...
///
template <typename Iterator>
MUDA_THREAD_ONLY inline typename eastl::is_iterator_wrapper_helper<Iterator, eastl::is_generic_iterator<Iterator>::value>::iterator_type unwrap_generic_iterator(
    Iterator it)
{
    // get_unwrapped(it) -> it.unwrap() which is equivalent to `it.base()` for generic_iterator and to `it` otherwise.
    return eastl::is_iterator_wrapper_helper<Iterator, eastl::is_generic_iterator<Iterator>::value>::get_unwrapped(
        it);
}
