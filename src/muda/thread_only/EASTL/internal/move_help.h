...
#define EASTL_FORWARD_INLINE(T, x)                                             \
    eastl::forward<T>(x)  // Need to investigate how to properly make a macro for this. (eastl::is_reference<T>::value ? static_cast<T&&>(static_cast<T&>(x)) : static_cast<T&&>(x))
...
//
template <typename T>
MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR T&& forward(typename eastl::remove_reference<T>::type& x) EA_NOEXCEPT
{
...
//
template <typename T>
MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR typename eastl::remove_reference<T>::type&& move(T&& x) EA_NOEXCEPT
{
...
template <typename T>
MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR typename eastl::conditional<!eastl::is_nothrow_move_constructible<T>::value && eastl::is_copy_constructible<T>::value, const T&, T&&>::type move_if_noexcept(
    T& x) EA_NOEXCEPT
{
    return eastl::move(x);
}
#else
template <typename T>
MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR T&& move_if_noexcept(T& x) EA_NOEXCEPT
{
    return eastl::move(x);
}
#endif
