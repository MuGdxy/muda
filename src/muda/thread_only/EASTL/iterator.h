...
namespace eastl
{
	...
	struct is_iterator_wrapper_helper
	{
		...
		MUDA_THREAD_ONLY static iterator_type get_unwrapped(Iterator it) { return it; }
	};
	...
	struct is_iterator_wrapper_helper<Iterator, true>
	{
		...
		MUDA_THREAD_ONLY static iterator_type get_unwrapped(Iterator it)
        {
            return it.unwrap();
        }
	};
	...
	template <typename Iterator>
    MUDA_THREAD_ONLY inline typename is_iterator_wrapper_helper<Iterator, eastl::is_iterator_wrapper<Iterator>::value>::iterator_type unwrap_iterator(
        Iterator it)
	...
	public:
        MUDA_THREAD_ONLY  EA_CPP14_CONSTEXPR reverse_iterator()  // It's important that we construct mIterator, because if Iterator
			: mIterator() { }   // is a pointer, there's a difference between doing it and not.

		MUDA_THREAD_ONLY  EA_CPP14_CONSTEXPR explicit reverse_iterator(iterator_type i)
			: mIterator(i) { }

		MUDA_THREAD_ONLY  EA_CPP14_CONSTEXPR reverse_iterator(const reverse_iterator& ri)
			: mIterator(ri.mIterator) { }

		template <typename U>
        MUDA_THREAD_ONLY  EA_CPP14_CONSTEXPR reverse_iterator(const reverse_iterator<U>& ri)
			: mIterator(ri.base()) { }
		...
		template <typename U>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR reverse_iterator<Iterator>& operator=(const reverse_iterator<U>& ri)
			{ mIterator = ri.base(); return *this; }

		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR iterator_type base() const
			{ return mIterator; }

		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR reference operator*() const
		{
			iterator_type i(mIterator);
			return *--i;
		}

		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pointer operator->() const
			{ return &(operator*()); }

		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR reverse_iterator& operator++()
			{ --mIterator; return *this; }

		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR reverse_iterator operator++(int)
		{
		...
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR reverse_iterator operator+(difference_type n) const
			{ return reverse_iterator(mIterator - n); }

		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR reverse_iterator& operator+=(difference_type n)
		...
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR reference operator[](difference_type n) const
	...
	private:
		...
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR typename eastl::enable_if<eastl::is_iterator_wrapper<U>::value, reverse_iterator<base_wrapped_iterator_type>>::type unwrap() const
	...
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline bool
	...
	template <typename Iterator1, typename Iterator2>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline typename reverse_iterator<Iterator1>::difference_type
	operator-(const reverse_iterator<Iterator1>& a, const reverse_iterator<Iterator2>& b)
		{ return b.base() - a.base(); }


	template <typename Iterator>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline reverse_iterator<Iterator>
	...
	public:
        MUDA_THREAD_ONLY move_iterator()
		  : mIterator()
		{
		}

		MUDA_THREAD_ONLY explicit move_iterator(iterator_type mi)
		  : mIterator(mi) { }

		template<typename U>
        MUDA_THREAD_ONLY move_iterator(const move_iterator<U>& mi)
		  : mIterator(mi.base())
		{
		}

		MUDA_THREAD_ONLY iterator_type base() const
			{ return mIterator; }

		MUDA_THREAD_ONLY reference operator*() const
        {
            return static_cast<reference>(*mIterator);
        }

		MUDA_THREAD_ONLY pointer operator->() const
			{ return mIterator; }

		MUDA_THREAD_ONLY move_iterator& operator++()
		{
			++mIterator;
			return *this;
		}

		MUDA_THREAD_ONLY move_iterator operator++(int)
		{
		...
		MUDA_THREAD_ONLY move_iterator operator+(difference_type n) const
			{ return move_iterator(mIterator + n); }

		MUDA_THREAD_ONLY move_iterator& operator+=(difference_type n)
		{
			mIterator += n;
			return *this;
		}
		...
		MUDA_THREAD_ONLY reference operator[](difference_type n) const
			{ return eastl::move(mIterator[n]); }

	private:
		// Unwrapping interface, not part of the public API.
      MUDA_THREAD_ONLY iterator_type unwrap() const
			{ return mIterator; }
	...
	// To consider: merge the conditional expression usage here with the one used by move_if_noexcept, as they are the same condition.
        ...
        MUDA_THREAD_ONLY inline IteratorType make_move_if_noexcept_iterator(Iterator i)
			{ return IteratorType(i); }
	#else
		// Else there are no exceptions and thus we always return a move_iterator.
		template <typename Iterator>
        MUDA_THREAD_ONLY inline eastl::move_iterator<Iterator> make_move_if_noexcept_iterator(Iterator i)
	...
	///
	template <typename Iterator>
    MUDA_THREAD_ONLY inline typename eastl::is_iterator_wrapper_helper<Iterator, eastl::is_move_iterator<Iterator>::value>::iterator_type unwrap_move_iterator(
        Iterator it)
	{
	...
	public:
		...
		MUDA_THREAD_ONLY explicit back_insert_iterator(Container& x)
			: container(x) { }

		MUDA_THREAD_ONLY back_insert_iterator& operator=(const_reference value)
			{ container.push_back(value); return *this; }

		MUDA_THREAD_ONLY back_insert_iterator& operator=(typename Container::value_type&& value)
			{ container.push_back(eastl::move(value)); return *this; }

		MUDA_THREAD_ONLY back_insert_iterator& operator*()
			{ return *this; }

		MUDA_THREAD_ONLY back_insert_iterator& operator++()
			{ return *this; } // This is by design.

		MUDA_THREAD_ONLY back_insert_iterator operator++(int)
			{ return *this; } // This is by design.

	protected:
            MUDA_THREAD_ONLY void operator=(const this_type&) {
	...
	///
	template <typename Container>
    MUDA_THREAD_ONLY inline back_insert_iterator<Container>
	...
	public:
		...
		MUDA_THREAD_ONLY explicit front_insert_iterator(Container& x)
			: container(x) { }

		MUDA_THREAD_ONLY front_insert_iterator& operator=(const_reference value)
			{ container.push_front(value); return *this; }

		MUDA_THREAD_ONLY front_insert_iterator& operator*()
			{ return *this; }

		MUDA_THREAD_ONLY front_insert_iterator& operator++()
			{ return *this; } // This is by design.

		MUDA_THREAD_ONLY front_insert_iterator operator++(int)
			{ return *this; } // This is by design.
	...
	///
	template <typename Container>
    MUDA_THREAD_ONLY inline front_insert_iterator<Container>
	...
	public:
      ...
      MUDA_THREAD_ONLY insert_iterator& operator=(const insert_iterator& x)
		{
		...
		MUDA_THREAD_ONLY insert_iterator(Container& x, iterator_type itNew)
			: container(x), it(itNew) {}

		MUDA_THREAD_ONLY insert_iterator& operator=(const_reference value)
		{
		...
		MUDA_THREAD_ONLY insert_iterator& operator*()
			{ return *this; }

		MUDA_THREAD_ONLY insert_iterator& operator++()
			{ return *this; } // This is by design.

		MUDA_THREAD_ONLY insert_iterator& operator++(int)
			{ return *this; } // This is by design.
    ...
    MUDA_THREAD_ONLY inline eastl::insert_iterator<Container>
	...
	struct advance_bi_impl
	{
        ...
        MUDA_THREAD_ONLY static void advance_impl(BidirectionalIterator& i, Distance n)  // Specialization for unsigned distance type.
		{
			while(n--)
				++i;
		}
	...
	{
		template <typename BidirectionalIterator, typename Distance>
        MUDA_THREAD_ONLY static void advance_impl(BidirectionalIterator& i, Distance n)  // Specialization for signed distance type.
		{
			if(n > 0)
			{
				while(n--)
	...
	//
	template<typename InputIterator>
    MUDA_THREAD_ONLY inline InputIterator
	...
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR auto data(Container& c) -> decltype(c.data())
		{ return c.data(); }

	template <class Container>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR auto data(const Container& c) -> decltype(c.data())
		{ return c.data(); }

	template <class T, size_t N>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR T* data(T(&array)[N]) EA_NOEXCEPT
		{ return array; }

	template <class E>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR const E* data(std::initializer_list<E> il) EA_NOEXCEPT
		{ return il.begin(); }
	...
	//
	template <class C>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR auto size(const C& c) -> decltype(c.size())
		{ return c.size(); }

	template <class T, size_t N>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR size_t size(const T (&)[N]) EA_NOEXCEPT
		{ return N; }
	...
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR ptrdiff_t ssize(const T(&)[N]) EA_NOEXCEPT
		{ return N; }

	template <class C>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR auto ssize(const C& c)
	...
	//
	template <class Container>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR auto empty(const Container& c) -> decltype(c.empty())
		{ return c.empty(); }

	template <class T, size_t N>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR bool empty(const T (&)[N]) EA_NOEXCEPT
		{ return false; }

	template <class E>
	MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR bool empty(std::initializer_list<E> il) EA_NOEXCEPT
		{ return il.size() == 0; }
	...
	//
		...
		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto begin(Container& container) -> decltype(container.begin())
		{
			return container.begin();
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto begin(const Container& container) -> decltype(container.begin())
		{
			return container.begin();
		}

		template<typename T, size_t arraySize>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline T* begin(T (&arrayObject)[arraySize]) EA_NOEXCEPT
		{
			return arrayObject;
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto cbegin(const Container& container) -> decltype(eastl::begin(container))
		{
			return eastl::begin(container);
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto end(Container& container) -> decltype(container.end())
		{
			return container.end();
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto end(const Container& container) -> decltype(container.end())
		{
			return container.end();
		}

		template<typename T, size_t arraySize>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline T* end(T (&arrayObject)[arraySize]) EA_NOEXCEPT
		{
			return (arrayObject + arraySize);
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto cend(const Container& container) -> decltype(eastl::end(container))
		{
			return eastl::end(container);
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto rbegin(Container& container) -> decltype(container.rbegin())
		{
			return container.rbegin();
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto rbegin(const Container& container) -> decltype(container.rbegin())
		{
			return container.rbegin();
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto rend(Container& container) -> decltype(container.rend())
		{
			return container.rend();
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto rend(const Container& container) -> decltype(container.rend())
		{
			return container.rend();
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto crbegin(const Container& container) -> decltype(eastl::rbegin(container))
		{
			return container.rbegin();
		}

		template <typename Container>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline auto crend(const Container& container) -> decltype(eastl::rend(container))
		{
			return container.rend();
		}
		...
		template <typename T, size_t arraySize>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline reverse_iterator<T*> rbegin(T (&arrayObject)[arraySize])
		{
			return reverse_iterator<T*>(arrayObject + arraySize);
		}

		template <typename T, size_t arraySize>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline reverse_iterator<T*> rend(T (&arrayObject)[arraySize])
		{
			return reverse_iterator<T*>(arrayObject);
		}

		template <typename E>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline reverse_iterator<const E*> rbegin(std::initializer_list<E> ilist)
		{
			return eastl::reverse_iterator<const E*>(ilist.end());
		}

		template <typename E>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline reverse_iterator<const E*> rend(std::initializer_list<E> ilist)
		{
			return eastl::reverse_iterator<const E*>(ilist.begin());
		}

		template <typename Iterator>
		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR reverse_iterator<Iterator> make_reverse_iterator(Iterator i)
