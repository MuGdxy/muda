...
namespace eastl
{
	...
	/// 
	template <typename T>
MUDA_THREAD_ONLY eastl::pair<T*, ptrdiff_t> get_temporary_buffer(
    ptrdiff_t n, size_t alignment = 1, size_t alignmentOffset = 0, const char* pName = EASTL_TEMP_DEFAULT_NAME)
	{
	...
	/// 
	template <typename T>
    MUDA_THREAD_ONLY void return_temporary_buffer(T* p, ptrdiff_t n = 0)
	{
	...
	public:
		...
		MUDA_THREAD_ONLY late_constructed() EA_NOEXCEPT  // In the case of the late_constructed instance being at global scope, we rely on the 
		  : mStorage(), mpValue(nullptr) {}  // compiler executing this constructor or placing the instance in auto-zeroed-at-startup memory.

		MUDA_THREAD_ONLY ~late_constructed()
		{
        ...
        MUDA_THREAD_ONLY void construct(Args&&... args)
		{
			if(!mpValue)
				mpValue = new (&mStorage) value_type(eastl::forward<Args>(args)...);
		}

		MUDA_THREAD_ONLY bool is_constructed() const EA_NOEXCEPT
			{ return mpValue != nullptr; }

		MUDA_THREAD_ONLY void destruct()
		{
			if(mpValue)
			{
				(*mpValue).~value_type();
		...
		MUDA_THREAD_ONLY value_type& operator*() EA_NOEXCEPT
		{
			if(!mpValue)
				construct();
		...
		MUDA_THREAD_ONLY const value_type& operator*() const EA_NOEXCEPT
		{
			if(!mpValue)
				construct();
		...
		MUDA_THREAD_ONLY value_type* get() EA_NOEXCEPT
		{
			if(!mpValue)
				construct(); 
			return mpValue;
		...
		MUDA_THREAD_ONLY const value_type* get() const EA_NOEXCEPT
		{
			if(!mpValue)
				construct(); 
			return mpValue;
	...
	public:
		...
		MUDA_THREAD_ONLY typename base_type::value_type& operator*() EA_NOEXCEPT
			{ EASTL_ASSERT(base_type::mpValue); return *base_type::mpValue; }

		MUDA_THREAD_ONLY const typename base_type::value_type& operator*() const EA_NOEXCEPT
			{ EASTL_ASSERT(base_type::mpValue); return *base_type::mpValue; }
		...
		MUDA_THREAD_ONLY typename base_type::value_type* get() EA_NOEXCEPT
			{ return base_type::mpValue; }

		MUDA_THREAD_ONLY const typename base_type::value_type* get() const EA_NOEXCEPT
			{ return base_type::mpValue; }
	};
	...
	public:
        MUDA_THREAD_ONLY explicit raw_storage_iterator(OutputIterator iterator)
		  : mIterator(iterator) 
		{
		}

		MUDA_THREAD_ONLY raw_storage_iterator& operator*() 
		{
			return *this;
		}

		MUDA_THREAD_ONLY raw_storage_iterator& operator=(const T& value)
		{
			::new(eastl::addressof(*mIterator)) T(value);
			return *this;
		}

		MUDA_THREAD_ONLY raw_storage_iterator<OutputIterator, T>& operator++()
		{
			++mIterator;
			return *this;
		}

		MUDA_THREAD_ONLY raw_storage_iterator<OutputIterator, T> operator++(int)
		{
	...
	namespace Internal
	{
		...
		struct uninitialized_relocate_impl
		{
            ...
            MUDA_THREAD_ONLY static ForwardIteratorDest do_move_start(
			...
			template <typename ForwardIterator, typename ForwardIteratorDest>
            MUDA_THREAD_ONLY static ForwardIteratorDest do_move_commit(
                ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)  //throw()
			{
				typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
				for(; first != last; ++first, ++dest)
					(*first).~value_type();
			...
			template <typename ForwardIterator, typename ForwardIteratorDest>
            MUDA_THREAD_ONLY static ForwardIteratorDest do_move_abort(
                ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)  //throw()
			{
				typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
				for(; first != last; ++first, ++dest)
		...
		{
			template <typename T>
            MUDA_THREAD_ONLY static T* do_move_start(T* first, T* last, T* dest)
			{
			...
			template <typename T>
            MUDA_THREAD_ONLY static T* do_move_commit(T* first, T* last, T* dest)
			{
				return dest + (last - first);
			}

			template <typename T>
            MUDA_THREAD_ONLY static T* do_move_abort(T* first, T* last, T* dest)
			{
				return dest + (last - first);
			}
		};
	...
	///
	template <typename ForwardIterator, typename ForwardIteratorDest>
    MUDA_THREAD_ONLY inline ForwardIteratorDest uninitialized_relocate_start(
        ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
	{
	...
	template <typename ForwardIterator, typename ForwardIteratorDest>
    MUDA_THREAD_ONLY inline ForwardIteratorDest uninitialized_relocate_commit(
        ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::iterator_category IC;
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type_input;
	...
	template <typename ForwardIterator, typename ForwardIteratorDest>
    MUDA_THREAD_ONLY inline ForwardIteratorDest uninitialized_relocate_abort(
        ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::iterator_category IC;
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type_input;
	...
	///
	template <typename ForwardIterator, typename ForwardIteratorDest>
    MUDA_THREAD_ONLY inline ForwardIteratorDest uninitialized_relocate(
        ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
	{
	...
	namespace Internal
	{
        ...
        MUDA_THREAD_ONLY inline ForwardIterator uninitialized_copy_impl(
            InputIterator first, InputIterator last, ForwardIterator dest, true_type)
		{
	...
	///
	template <typename InputIterator, typename ForwardIterator>
    MUDA_THREAD_ONLY inline ForwardIterator uninitialized_copy(InputIterator first,
                                                               InputIterator last,
	...
	namespace Internal
	{
		...
		{
            MUDA_THREAD_ONLY static ForwardIterator impl(InputIterator first, Count n, ForwardIterator dest)
			{
				typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
				ForwardIterator currentDest(dest);
		...
		{
            MUDA_THREAD_ONLY static inline ForwardIterator impl(InputIterator first,
                                                                Count n,
	...
	template<typename InputIterator, typename Count, typename ForwardIterator>
    MUDA_THREAD_ONLY inline ForwardIterator uninitialized_copy_n(InputIterator first,
                                                                 Count n,
                                                                 ForwardIterator dest)
	{
    ...
    MUDA_THREAD_ONLY inline Result uninitialized_copy_ptr(First first, Last last, Result result)
	{
	...
	{
		template <typename InputIterator, typename ForwardIterator>
        MUDA_THREAD_ONLY inline ForwardIterator uninitialized_move_impl(
            InputIterator first, InputIterator last, ForwardIterator dest, true_type)
		{
	...
	template <typename First, typename Last, typename Result>
    MUDA_THREAD_ONLY inline Result uninitialized_move_ptr(First first, Last last, Result dest)
	{
		typedef typename eastl::iterator_traits<generic_iterator<Result, void> >::value_type value_type;
	...
	///
	template <typename InputIterator, typename ForwardIterator>
    MUDA_THREAD_ONLY inline ForwardIterator uninitialized_move(InputIterator first,
                                                               InputIterator last,
                                                               ForwardIterator dest)
	{
	...
	///
	template <typename InputIterator, typename ForwardIterator>
    MUDA_THREAD_ONLY inline ForwardIterator uninitialized_move_if_noexcept(
        InputIterator first, InputIterator last, ForwardIterator dest)
	{
	...
	///
	template <typename First, typename Last, typename Result>
    MUDA_THREAD_ONLY inline Result uninitialized_move_ptr_if_noexcept(First first,
                                                                      Last last,
                                                                      Result dest)
	{
		#if EASTL_EXCEPTIONS_ENABLED
	...
	///
	template<typename InputIterator, typename Count, typename ForwardIterator>
    MUDA_THREAD_ONLY inline ForwardIterator uninitialized_move_n(InputIterator first,
                                                                 Count n,
                                                                 ForwardIterator dest)
	{
    ...
    MUDA_THREAD_ONLY inline void uninitialized_default_fill(ForwardIterator first,
	...
	namespace Internal
	{
        ...
        MUDA_THREAD_ONLY inline void uninitialized_default_fill_n_impl(ForwardIterator first,
                                                                       Count n,
                                                                       false_type)
		{
			typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
	...
	template <typename ForwardIterator, typename Count>
    MUDA_THREAD_ONLY inline void uninitialized_default_fill_n(ForwardIterator first, Count n)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
	...
	///
	template <typename ForwardIterator>
    MUDA_THREAD_ONLY inline void uninitialized_default_construct(ForwardIterator first,
                                                                 ForwardIterator last)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
		ForwardIterator currentDest(first);
	...
	///
	template <typename ForwardIterator, typename Count>
    MUDA_THREAD_ONLY inline ForwardIterator uninitialized_default_construct_n(ForwardIterator first,
                                                                              Count n)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
		ForwardIterator currentDest(first);
	...
	{
		template <typename ForwardIterator, typename T>
        MUDA_THREAD_ONLY inline void uninitialized_fill_impl(ForwardIterator first,
                                                             ForwardIterator last,
                                                             const T& value,
                                                             true_type)
		{
		...
		template <typename ForwardIterator, typename T>
        MUDA_THREAD_ONLY void uninitialized_fill_impl(ForwardIterator first,
                                                      ForwardIterator last,
                                                      const T&        value,
                                                      false_type)
		{
	...
	template <typename ForwardIterator, typename T>
    MUDA_THREAD_ONLY inline void uninitialized_fill(ForwardIterator first,
                                                    ForwardIterator last,
                                                    const T&        value)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
    ...
    MUDA_THREAD_ONLY void uninitialized_value_construct(ForwardIterator first, ForwardIterator last)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
		ForwardIterator currentDest(first);
    ...
    MUDA_THREAD_ONLY inline ForwardIterator uninitialized_value_construct_n(ForwardIterator first, Count n)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
		ForwardIterator currentDest(first);
	...
	///
	template <typename T>
    MUDA_THREAD_ONLY inline void uninitialized_fill_ptr(T* first, T* last, const T& value)
	{
	...
	{
		template <typename ForwardIterator, typename Count, typename T>
        MUDA_THREAD_ONLY inline void uninitialized_fill_n_impl(ForwardIterator first,
                                                               Count    n,
                                                               const T& value,
                                                               true_type)
		{
	...
	template <typename ForwardIterator, typename Count, typename T>
    MUDA_THREAD_ONLY inline void uninitialized_fill_n(ForwardIterator first, Count n, const T& value)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
    ...
    MUDA_THREAD_ONLY inline void uninitialized_fill_n_ptr(T* first, Count n, const T& value)
	{
    ...
    MUDA_THREAD_ONLY inline void uninitialized_copy_fill(InputIterator   first1,
                                                         InputIterator   last1,
	...
	///
	template <typename InputIterator, typename ForwardIterator, typename T>
    MUDA_THREAD_ONLY inline void uninitialized_move_fill(InputIterator   first1,
                                                         InputIterator   last1,
										ForwardIterator first2, ForwardIterator last2, const T& value)
	{
    ...
    MUDA_THREAD_ONLY inline ForwardIterator
	...
	/// 
	template <typename T>
    MUDA_THREAD_ONLY inline void destruct(T* p)
	{
	...
	//
	template <typename ForwardIterator>
    MUDA_THREAD_ONLY inline void destruct_impl(ForwardIterator /*first*/,
                                               ForwardIterator /*last*/,
	...
	///
	template <typename ForwardIterator>
    MUDA_THREAD_ONLY inline void destruct(ForwardIterator first, ForwardIterator last)
	{
		typedef typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
	...
	/// 
	template <typename T>
    MUDA_THREAD_ONLY inline void destroy_at(T* p)
	{
		EA_UNUSED(p);
		p->~T();
	}
	...
	///
	template <typename ForwardIterator>
    MUDA_THREAD_ONLY inline void destroy(ForwardIterator first, ForwardIterator last)
	{
		for (; first != last; ++first)
    ...
    MUDA_THREAD_ONLY inline ForwardIterator destroy_n(ForwardIterator first, Size n)
	{
		for (; n > 0; ++first, --n)
			eastl::destroy_at(eastl::addressof(*first));
	...
	MUDA_THREAD_ONLY inline void* align(size_t alignment, size_t size, void*& ptr, size_t& space)
	{
		if(space >= size)
		{
	...
	MUDA_THREAD_ONLY inline void* align_advance(size_t  alignment,
                                                size_t  size,
                                                void*   ptr,
                                                size_t  space,
                                                void**  ptrAdvanced  = NULL,
	...
	private:
		template <typename>
      MUDA_THREAD_ONLY static eastl::no_type test(...);

		template <typename U>
        MUDA_THREAD_ONLY static eastl::yes_type test(typename U::allocator_type* = NULL);
	...
	namespace Internal
	{
		...
		private:
			...
			MUDA_THREAD_ONLY static eastl::no_type  test(...);
			template <typename U> 
			MUDA_THREAD_ONLY static eastl::yes_type test(typename U::element_type* = 0);
		public:
		...
		private:
            ...
            MUDA_THREAD_ONLY static eastl::no_type test(...);
          template <typename U>
          MUDA_THREAD_ONLY static eastl::yes_type test(typename U::difference_type* = 0);
		public:
			static const bool value = sizeof((test<Pointer>(0))) == sizeof(eastl::yes_type);
		};
	...
	public:
            MUDA_THREAD_ONLY static pointer pointer_to(
                typename eastl::conditional<eastl::is_void<element_type>::value, void, element_type>::type& r)  // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is unspecified; otherwise, it is T&.
	...
	template<class T>
    MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR T* to_address(T* p) noexcept
	{
    ...
    MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR auto to_address(const Ptr& ptr) noexcept
        -> decltype(eastl::pointer_traits<Ptr>::to_address(ptr))
	{
