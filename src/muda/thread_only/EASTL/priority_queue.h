...
namespace eastl
{
	...
	public:
		MUDA_THREAD_ONLY priority_queue();
		...
		MUDA_THREAD_ONLY explicit priority_queue(
		...
		MUDA_THREAD_ONLY explicit priority_queue(const compare_type& compare);
        MUDA_THREAD_ONLY explicit priority_queue(const compare_type& compare,
                                                 container_type&&    x);

		MUDA_THREAD_ONLY priority_queue(const compare_type&   compare,
                                        const container_type& x);
        MUDA_THREAD_ONLY priority_queue(std::initializer_list<value_type> ilist,
                                        const compare_type& compare = compare_type());  // C++11 doesn't specify that std::priority_queue has initializer list support.

		template <typename InputIterator>
        MUDA_THREAD_ONLY priority_queue(InputIterator first, InputIterator last);

		template <typename InputIterator>
        MUDA_THREAD_ONLY priority_queue(InputIterator       first,
                                        InputIterator       last,
		...
		MUDA_THREAD_ONLY bool empty() const;
        MUDA_THREAD_ONLY size_type size() const;

		MUDA_THREAD_ONLY const_reference top() const;

		MUDA_THREAD_ONLY void push(const value_type& value);

		MUDA_THREAD_ONLY void push(value_type&& x);

		template <class... Args>
        MUDA_THREAD_ONLY void emplace(Args&&... args);

		MUDA_THREAD_ONLY void pop();

		MUDA_THREAD_ONLY void pop(value_type& value);  // Extension to the C++11 Standard that allows popping a move-only type (e.g. unique_ptr).

		MUDA_THREAD_ONLY void change(size_type n);  /// Moves the item at the given array index to a new location based on its current priority.
        MUDA_THREAD_ONLY void remove(size_type n);  /// Removes the item at the given array index.

		MUDA_THREAD_ONLY container_type& get_container();
        MUDA_THREAD_ONLY const container_type& get_container() const;

		MUDA_THREAD_ONLY void swap(this_type& x)
		...
		MUDA_THREAD_ONLY bool validate() const;
