...
namespace eastl
{
	...
	template <typename T> 
	MUDA_THREAD_ONLY inline void swap(T& a, T& b)
    ...
    MUDA_THREAD_ONLY eastl::unused swap(eastl::argument_sink, eastl::argument_sink);
		#else
	...
	struct iter_swap_impl
	{
        ...
        MUDA_THREAD_ONLY static void iter_swap(ForwardIterator1 a, ForwardIterator2 b)
		{
	...
	///
	template <typename ForwardIterator1, typename ForwardIterator2>
    MUDA_THREAD_ONLY inline void iter_swap(ForwardIterator1 a, ForwardIterator2 b)
	{
		typedef typename eastl::iterator_traits<ForwardIterator1>::value_type value_type_a;
	...
	///
	template <typename ForwardIterator1, typename ForwardIterator2>
    MUDA_THREAD_ONLY inline ForwardIterator2
    ...
    MUDA_THREAD_ONLY inline T exchange(T& obj, U&& new_value)
	{
	...
	///
	template <class T>
    MUDA_THREAD_ONLY EA_CONSTEXPR typename eastl::add_const<T>::type& as_const(T& t) EA_NOEXCEPT
		{ return t; }
	...
	template <class T>
        MUDA_THREAD_ONLY void as_const(const T&&) = delete;
	...
	{
		template <typename T>
        MUDA_THREAD_ONLY inline bool operator!=(const T& x, const T& y)
			{ return !(x == y); }
	...
	///
	template <typename T, typename U>
    MUDA_THREAD_ONLY EA_CONSTEXPR bool cmp_equal(const T x, const U y) EA_NOEXCEPT
	{
	...
	template <typename T, typename U>
	MUDA_THREAD_ONLY EA_CONSTEXPR bool cmp_not_equal(const T x, const U y) EA_NOEXCEPT
		{ return !eastl::cmp_equal(x, y); }


	template <typename T, typename U>
	MUDA_THREAD_ONLY EA_CONSTEXPR bool cmp_less(const T x, const U y) EA_NOEXCEPT
	{
		static_assert(eastl::is_integral_v<T> && !eastl::is_same_v<eastl::remove_cv_t<T>, bool> && !eastl::is_same_v<eastl::remove_cv_t<T>, char>);
		static_assert(eastl::is_integral_v<U> && !eastl::is_same_v<eastl::remove_cv_t<U>, bool> && !eastl::is_same_v<eastl::remove_cv_t<U>, char>);
	...
	template <typename T, typename U>
	MUDA_THREAD_ONLY EA_CONSTEXPR bool cmp_greater(const T x, const U y) EA_NOEXCEPT
		{ return eastl::cmp_less(y, x); }


	template <typename T, typename U>
	MUDA_THREAD_ONLY EA_CONSTEXPR bool cmp_less_equal(const T x, const U y) EA_NOEXCEPT
		{ return !eastl::cmp_greater(x, y); }


	template <typename T, typename U>
	MUDA_THREAD_ONLY EA_CONSTEXPR bool cmp_greater_equal(const T x, const U y) EA_NOEXCEPT
		{ return !eastl::cmp_less(x, y); }


	template <typename T, typename U>
	MUDA_THREAD_ONLY EA_CONSTEXPR bool in_range(const U x) EA_NOEXCEPT
	{
		static_assert(eastl::is_integral_v<T> && !eastl::is_same_v<eastl::remove_cv_t<T>, bool> && !eastl::is_same_v<eastl::remove_cv_t<T>, char>);
		static_assert(eastl::is_integral_v<U> && !eastl::is_same_v<eastl::remove_cv_t<U>, bool> && !eastl::is_same_v<eastl::remove_cv_t<U>, char>);
	...
	struct pair
	{
        ...
        MUDA_THREAD_ONLY EA_CONSTEXPR pair()
		    : first(), second()
		{
		}
        ...
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(const TT1& x)
		    : first(x), second()
		{
		}
        ...
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(T1&& x)
	...
	// https://bugs.llvm.org/show_bug.cgi?id=38374
        ...
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(pair_first_construct_t, const TT1& x)
		    : first(x), second()
		{
		}
        ...
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(pair_first_construct_t, T1&& x)
				: first(eastl::move(x)), second()
			{
			}
		#endif
		...
		template <
            ...
            MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(const T1& x, const T2& y)
		    : first(x), second(y)
		{
		}

		MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(pair&& p)    = default;
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(const pair&) = default;

		template <
		    typename U,
		    typename V,
		    class = eastl::enable_if_t<eastl::is_convertible_v<const U&, T1> && eastl::is_convertible_v<const V&, T2>>>
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(const pair<U, V>& p)
        ...
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(U&& u, V&& v)
        ...
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(U&& x, const T2& y)
			: first(eastl::forward<U>(x)), second(y)
		{
		}

		template <typename V, typename = eastl::enable_if_t<eastl::is_convertible_v<V, T2>>>
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(const T1& x, V&& y)
		...
		template <typename U,
		          typename V,
		          typename = eastl::enable_if_t<eastl::is_convertible_v<U, T1> && eastl::is_convertible_v<V, T2>>>
        MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR pair(pair<U, V>&& p)
        ...
        MUDA_THREAD_ONLY pair(eastl::piecewise_construct_t pwc,
	...
	private:
      ...
      MUDA_THREAD_ONLY pair(eastl::piecewise_construct_t,
			 eastl::tuple<Args1...> first_args,
			 eastl::tuple<Args2...> second_args,
	...
	public:
        MUDA_THREAD_ONLY pair& operator=(const pair& p)
		...
		template <typename U,
		          typename V,
		          typename = eastl::enable_if_t<eastl::is_convertible_v<U, T1> && eastl::is_convertible_v<V, T2>>>
        MUDA_THREAD_ONLY pair& operator=(const pair<U, V>& p)
		{
			first = p.first;
			second = p.second;
			return *this;
		...
		MUDA_THREAD_ONLY pair& operator=(pair&& p)
		...
		template <typename U,
		          typename V,
		          typename = eastl::enable_if_t<eastl::is_convertible_v<U, T1> && eastl::is_convertible_v<V, T2>>>
        MUDA_THREAD_ONLY pair& operator=(pair<U, V>&& p)
		{
		...
		MUDA_THREAD_ONLY void swap(pair& p)
            EA_NOEXCEPT_IF(eastl::is_nothrow_swappable_v<T1>&& eastl::is_nothrow_swappable_v<T2>)
		{
	...
	struct use_first
	{
		...
		MUDA_THREAD_ONLY const result_type& operator()(const Pair& x) const
			{ return x.first; }
	};
	...
	template <typename T1, typename T2>
    MUDA_THREAD_ONLY EA_CPP14_CONSTEXPR inline bool operator==(const pair<T1, T2>& a,
                                                               const pair<T1, T2>& b)
	{
	...
	template <typename T1, typename T2>
    MUDA_THREAD_ONLY EA_CONSTEXPR inline std::common_comparison_category_t<synth_three_way_result<T1>, synth_three_way_result<T2>> operator<=>(
	...
	template <typename T1, typename T2>
	MUDA_THREAD_ONLY EA_CONSTEXPR inline bool operator!=(const pair<T1, T2>& a, const pair<T1, T2>& b)
	{
		return !(a == b);
	}
	...
	///
	template <typename T1, typename T2>
	MUDA_THREAD_ONLY EA_CONSTEXPR inline pair<typename eastl::remove_reference_wrapper<typename eastl::decay<T1>::type>::type, 
	...
	#if defined(_MSC_VER)
		template <typename T1, typename T2>
		MUDA_THREAD_ONLY EA_CONSTEXPR inline pair<T1, T2> make_pair(
			const T1& a,
			const T2& b,
	...
	make_pair_ref(T1&& a, T2&& b)
	{
		...
		{
			template <typename T1, typename T2>
			MUDA_THREAD_ONLY static EA_CONSTEXPR T1& getInternal(pair<T1, T2>& p)
			{
				return p.first;
			}

			template <typename T1, typename T2>
			MUDA_THREAD_ONLY static EA_CONSTEXPR const T1& getInternal(const pair<T1, T2>& p)
			{
				return p.first;
			}
		...
		{
			template <typename T1, typename T2>
			MUDA_THREAD_ONLY static EA_CONSTEXPR T2& getInternal(pair<T1, T2>& p)
			{
				return p.second;
			}

			template <typename T1, typename T2>
			MUDA_THREAD_ONLY static EA_CONSTEXPR const T2& getInternal(const pair<T1, T2>& p)
			{
				return p.second;
			}
        ...
        MUDA_THREAD_ONLY tuple_element_t<I, pair<T1, T2>>& get(pair<T1, T2>& p)
		{
			return GetPair<I>::getInternal(p);
		}

		template <size_t I, typename T1, typename T2>
        MUDA_THREAD_ONLY const tuple_element_t<I, pair<T1, T2>>& get(const pair<T1, T2>& p)
		{
			return GetPair<I>::getInternal(p);
		}
