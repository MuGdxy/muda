...
struct VectorBase
{
  ...
  protected:
    ...
    MUDA_THREAD_ONLY T*& internalCapacityPtr() EA_NOEXCEPT
    {
        return mCapacityAllocator.first();
    }
    MUDA_THREAD_ONLY T* const& internalCapacityPtr() const EA_NOEXCEPT
    {
        return mCapacityAllocator.first();
    }
    MUDA_THREAD_ONLY allocator_type& internalAllocator() EA_NOEXCEPT
    {
        return mCapacityAllocator.second();
    }
    MUDA_THREAD_ONLY const allocator_type& internalAllocator() const EA_NOEXCEPT
    {
        return mCapacityAllocator.second();
    }

  public:
    MUDA_THREAD_ONLY VectorBase();
    MUDA_THREAD_ONLY VectorBase(const allocator_type& allocator);
    MUDA_THREAD_ONLY VectorBase(size_type n, const allocator_type& allocator);
  ...
  protected:
    MUDA_THREAD_ONLY T* DoAllocate(size_type n);
    MUDA_THREAD_ONLY void DoFree(T* p, size_type n);
...
class vector : public VectorBase<T, Allocator>
{
  ...
  public:
    MUDA_THREAD_ONLY vector()
        EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(EASTL_VECTOR_DEFAULT_ALLOCATOR));
    MUDA_THREAD_ONLY explicit vector(const allocator_type& allocator) EA_NOEXCEPT;
    MUDA_THREAD_ONLY explicit vector(size_type n, const allocator_type& allocator = EASTL_VECTOR_DEFAULT_ALLOCATOR);
    MUDA_THREAD_ONLY vector(size_type         n,
           const value_type&     value,
           const allocator_type& allocator = EASTL_VECTOR_DEFAULT_ALLOCATOR);
    MUDA_THREAD_ONLY vector(const this_type& x);
    MUDA_THREAD_ONLY vector(const this_type& x, const allocator_type& allocator);
    MUDA_THREAD_ONLY vector(this_type&& x) EA_NOEXCEPT;
    MUDA_THREAD_ONLY vector(this_type&& x, const allocator_type& allocator);
    MUDA_THREAD_ONLY vector(std::initializer_list<value_type> ilist,
           const allocator_type& allocator = EASTL_VECTOR_DEFAULT_ALLOCATOR);

    template <typename InputIterator>
    MUDA_THREAD_ONLY vector(InputIterator first,
           InputIterator         last,
           const allocator_type& allocator = EASTL_VECTOR_DEFAULT_ALLOCATOR);
    ...
    MUDA_THREAD_ONLY void swap(this_type& x);  // TODO(c++17): noexcept(allocator_traits<Allocator>::propagate_on_container_move_assignment::value || allocator_traits<Allocator>::is_always_equal::value)

    MUDA_THREAD_ONLY void assign(size_type n, const value_type& value);

    template <typename InputIterator>
    MUDA_THREAD_ONLY void assign(InputIterator first, InputIterator last);

    MUDA_THREAD_ONLY void assign(std::initializer_list<value_type> ilist);

    MUDA_THREAD_ONLY iterator begin() EA_NOEXCEPT;
    MUDA_THREAD_ONLY const_iterator begin() const EA_NOEXCEPT;
    MUDA_THREAD_ONLY const_iterator cbegin() const EA_NOEXCEPT;

    MUDA_THREAD_ONLY iterator end() EA_NOEXCEPT;
    MUDA_THREAD_ONLY const_iterator end() const EA_NOEXCEPT;
    ...
    MUDA_THREAD_ONLY void resize(size_type n, const value_type& value);
    MUDA_THREAD_ONLY void resize(size_type n);
    MUDA_THREAD_ONLY void reserve(size_type n);
    MUDA_THREAD_ONLY void set_capacity(size_type n = base_type::npos);  // Revises the capacity to the user-specified value. Resizes the container to match the capacity if the requested capacity n is less than the current size. If n == npos then the capacity is reallocated (if necessary) such that capacity == size.
    MUDA_THREAD_ONLY void shrink_to_fit();  // C++11 function which is the same as set_capacity().

    MUDA_THREAD_ONLY pointer data() EA_NOEXCEPT;
    MUDA_THREAD_ONLY const_pointer data() const EA_NOEXCEPT;
    ...
    MUDA_THREAD_ONLY void push_back(const value_type& value);
    MUDA_THREAD_ONLY reference push_back();
    MUDA_THREAD_ONLY void*     push_back_uninitialized();
    MUDA_THREAD_ONLY void      push_back(value_type&& value);
    MUDA_THREAD_ONLY void      pop_back();

    template <class... Args>
    MUDA_THREAD_ONLY iterator emplace(const_iterator position, Args&&... args);

    template <class... Args>
    MUDA_THREAD_ONLY reference emplace_back(Args&&... args);

    MUDA_THREAD_ONLY iterator insert(const_iterator position, const value_type& value);
    MUDA_THREAD_ONLY iterator insert(const_iterator position, size_type n, const value_type& value);
    MUDA_THREAD_ONLY iterator insert(const_iterator position, value_type&& value);
    MUDA_THREAD_ONLY iterator insert(const_iterator                    position,
                                     std::initializer_list<value_type> ilist);

    template <typename InputIterator>
    MUDA_THREAD_ONLY iterator insert(const_iterator position, InputIterator first, InputIterator last);

    MUDA_THREAD_ONLY iterator erase_first(const T& value);
    MUDA_THREAD_ONLY iterator erase_first_unsorted(const T& value);  // Same as erase, except it doesn't preserve order, but is faster because it simply copies the last item in the vector over the erased position.
    MUDA_THREAD_ONLY reverse_iterator erase_last(const T& value);
    MUDA_THREAD_ONLY reverse_iterator erase_last_unsorted(const T& value);  // Same as erase, except it doesn't preserve order, but is faster because it simply copies the last item in the vector over the erased position.

    MUDA_THREAD_ONLY iterator erase(const_iterator position);
    MUDA_THREAD_ONLY iterator erase(const_iterator first, const_iterator last);
    MUDA_THREAD_ONLY iterator erase_unsorted(const_iterator position);  // Same as erase, except it doesn't preserve order, but is faster because it simply copies the last item in the vector over the erased position.

    MUDA_THREAD_ONLY reverse_iterator erase(const_reverse_iterator position);
    MUDA_THREAD_ONLY reverse_iterator erase(const_reverse_iterator first,
                                            const_reverse_iterator last);
    MUDA_THREAD_ONLY reverse_iterator erase_unsorted(const_reverse_iterator position);

    MUDA_THREAD_ONLY void clear() EA_NOEXCEPT;
    MUDA_THREAD_ONLY void reset_lose_memory() EA_NOEXCEPT;  // This is a unilateral reset to an initially empty state. No destructors are called, no deallocation occurs.
  ...
  protected:
    ...
    MUDA_THREAD_ONLY pointer DoRealloc(size_type       n,
    ...
    MUDA_THREAD_ONLY void DoInit(Integer n, Integer value, true_type);

    template <typename InputIterator>
    MUDA_THREAD_ONLY void DoInit(InputIterator first, InputIterator last, false_type);

    template <typename InputIterator>
    MUDA_THREAD_ONLY void DoInitFromIterator(InputIterator first,
                                             InputIterator last,
                                             EASTL_ITC_NS::input_iterator_tag);

    template <typename ForwardIterator>
    MUDA_THREAD_ONLY void DoInitFromIterator(ForwardIterator first,
                                             ForwardIterator last,
                                             EASTL_ITC_NS::forward_iterator_tag);

    template <typename Integer, bool bMove>
    MUDA_THREAD_ONLY void DoAssign(Integer n, Integer value, true_type);

    template <typename InputIterator, bool bMove>
    MUDA_THREAD_ONLY void DoAssign(InputIterator first, InputIterator last, false_type);

    MUDA_THREAD_ONLY void DoAssignValues(size_type n, const value_type& value);

    template <typename InputIterator, bool bMove>
    MUDA_THREAD_ONLY void DoAssignFromIterator(InputIterator first,
                                               InputIterator last,
                                               EASTL_ITC_NS::input_iterator_tag);

    template <typename RandomAccessIterator, bool bMove>
    MUDA_THREAD_ONLY void DoAssignFromIterator(RandomAccessIterator first,
                              RandomAccessIterator last,
                              EASTL_ITC_NS::random_access_iterator_tag);

    template <typename Integer>
    MUDA_THREAD_ONLY void DoInsert(const_iterator position, Integer n, Integer value, true_type);

    template <typename InputIterator>
    MUDA_THREAD_ONLY void DoInsert(const_iterator position,
                                   InputIterator  first,
                                   InputIterator  last,
                                   false_type);

    template <typename InputIterator>
    MUDA_THREAD_ONLY void DoInsertFromIterator(const_iterator position,
                              InputIterator  first,
                              InputIterator  last,
                              EASTL_ITC_NS::input_iterator_tag);
    ...
    MUDA_THREAD_ONLY void DoInsertValues(const_iterator    position,
                                         size_type         n,
                                         const value_type& value);

    MUDA_THREAD_ONLY void DoInsertValuesEnd(size_type n);  // Default constructs n values
    MUDA_THREAD_ONLY void DoInsertValuesEnd(size_type n, const value_type& value);

    template <typename... Args>
    MUDA_THREAD_ONLY void DoInsertValue(const_iterator position, Args&&... args);

    template <typename... Args>
    MUDA_THREAD_ONLY void DoInsertValueEnd(Args&&... args);

    MUDA_THREAD_ONLY void DoClearCapacity();

    MUDA_THREAD_ONLY void DoGrow(size_type n);

    MUDA_THREAD_ONLY void DoSwap(this_type& x);
                                                                           ...
                                                                           const_iterator last)
{
    ...
    if(first != last)
    {
        iterator const position =
            const_cast<value_type*>(eastl::move(const_cast<value_type*>(last),
                                                ...
                                                EASTL_ITC_NS::random_access_iterator_tag)
{
    ...
    if(n > size_type(internalCapacityPtr() - mpBegin))  // If n > capacity ...
    {
        pointer const pNewData =
            DoRealloc(n, first, last, should_move_or_copy_tag<bMove>());
        eastl::destruct(mpBegin, mpEnd);
        DoFree(mpBegin, (size_type)(internalCapacityPtr() - mpBegin));
    ...
    else  // else (size == capacity)
    {
        ...
        {  // To do: We are not handling exceptions properly below.  In particular we don't want to
            ...
            ::new((void*)(pNewData + nPosSize))
                value_type(eastl::forward<Args>(args)...);  // Because the old data is potentially being moved rather than copied, we need to move.
...
template <typename T, typename Allocator>
inline synth_three_way_result<T> operator<=>(const vector<T, Allocator>& a,
