  ...
  public:
    ...
    MUDA_GENERIC int map(int x) const noexcept
    {
        if constexpr(debugViewers)
            if(!(x >= 0 && x < dim_[0]))
            {
                muda_kernel_printf("mapper out of range: index=(%d) dim_=(%d)\n", x, dim_[0]);
  ...
  public:
    ...
    MUDA_GENERIC int map(int x, int y) const noexcept
    {
        if constexpr(debugViewers)
            ...
            {
                muda_kernel_printf("mapper out of range: index=(%d,%d) dim_=(%d,%d)\n",
                                   x,
                                   y,
                                   dim_[0],
                                   dim_[1]);
  ...
  public:
    ...
    MUDA_GENERIC int map(int x, int y, int z) const noexcept
    {
        if constexpr(debugViewers)
            ...
            {
                muda_kernel_printf("mapper out of range: index=(%d,%d,%d) dim_=(%d,%d,%d)\n",
                                   x,
                                   y,
                                   z,
                                   dim_[0],
